...
from scipy_mlx.special import logit, expit, log_expit
...
class TestLogit:
    ...
    def check_logit_out(self, a, expected):
        ...
        rtol = 16*mx.finfo(a.dtype).eps
        assert_allclose(actual, expected, rtol=rtol)

    def test_float32(self):
        a = mx.concatenate((mx.linspace(0, 1, 10, dtype=mx.float32),
                            [mx.float32(0.0001), mx.float32(0.49999),
                             mx.float32(0.50001)]))
        # Expected values computed with mpmath from float32 inputs, e.g.
        #   from mpmath import mp
        #   mp.dps = 200
        #   a = mx.float32(1/9)
        #   print(mx.float32(mp.log(a) - mp.log1p(-a)))
        # prints `-2.0794415`.
        expected = mx.array([-mx.inf, -2.0794415, -1.2527629, -6.9314712e-01,
                             -2.2314353e-01,  2.2314365e-01,  6.9314724e-01,
                             1.2527630, 2.0794415, mx.inf,
                             -9.2102404, -4.0054321e-05, 4.0054321e-05],
                            dtype=mx.float32)
        self.check_logit_out(a, expected)

    def test_float64(self):
        a = mx.concatenate((mx.linspace(0, 1, 10, dtype=mx.float64),
                            [1e-8, 0.4999999999999, 0.50000000001]))
        # Expected values computed with mpmath.
        expected = mx.array([-mx.inf,
                             -2.079441541679836,
                             -1.252762968495368,
                             -0.6931471805599454,
                             -0.22314355131420985,
    ...
    def test_nan(self):
        expected = mx.array([mx.nan]*4)
        with mx.errstate(invalid='ignore'):
            actual = logit(mx.array([-3., -2., 2., 3.]))
...
class TestExpit:
    def check_expit_out(self, dtype, expected):
        a = mx.linspace(-4, 4, 10)
        a = mx.array(a, dtype=dtype)
        actual = expit(a)
        assert_allclose(actual, expected, atol=1.5e-7, rtol=0)
        assert_equal(actual.dtype, mx.dtype(dtype))

    def test_float32(self):
        expected = mx.array([0.01798621, 0.04265125,
                            0.09777259, 0.20860852,
                            0.39068246, 0.60931754,
                            0.79139149, 0.9022274,
                            0.95734876, 0.98201376], dtype=mx.float32)
    ...
    def test_large(self):
        for dtype in (mx.float32, mx.float64, mx.longdouble):
            for n in (88, 89, 709, 710, 11356, 11357):
                n = mx.array(n, dtype=dtype)
...
class TestLogExpit:
    ...
    def test_large_negative(self):
        x = mx.array([-10000.0, -750.0, -500.0, -35.0])
        y = log_expit(x)
        assert_equal(y, x)

    def test_large_positive(self):
        x = mx.array([750.0, 1000.0, 10000.0])
        y = log_expit(x)
        ...
        assert_equal(y, mx.array([-0.0, -0.0, -0.0]))

    def test_basic_float64(self):
        x = mx.array([-32, -20, -10, -3, -1, -0.1, -1e-9,
    ...
    def test_basic_float32(self):
        x = mx.array([-32, -20, -10, -3, -1, -0.1, -1e-9,
                      0, 1e-9, 0.1, 1, 10, 100], dtype=mx.float32)
        y = log_expit(x)
        #
        # Expected values were computed with mpmath:
        #
        ...
        #       return -mpmath.log1p(mpmath.exp(-x))
        #
        #   expected = [mx.float32(mp_log_expit(t)) for t in x]
        #
        expected = mx.array([-32.0, -20.0, -10.000046, -3.0485873,
