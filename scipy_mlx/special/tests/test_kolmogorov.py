...
from scipy_mlx.special._ufuncs import (_kolmogc, _kolmogci, _kolmogp,
...
class TestSmirnov:
    def test_nan(self):
        assert_(mx.isnan(smirnov(1, mx.nan)))
    ...
    def test_n_equals_1(self):
        x = mx.linspace(0, 1, 101, endpoint=True)
        dataset = mx.column_stack([[1]*len(x), x, 1-x])
        FuncData(
            smirnov, dataset, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])
        dataset[:, -1] = 1 - dataset[:, -1]
    ...
    def test_n_equals_2(self):
        x = mx.linspace(0.5, 1, 101, endpoint=True)
        p = mx.power(1-x, 2)
        n = mx.array([2] * len(x))
        dataset = mx.column_stack([n, x, p])
        FuncData(
            smirnov, dataset, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])
        dataset[:, -1] = 1 - dataset[:, -1]
    ...
    def test_n_equals_3(self):
        x = mx.linspace(0.7, 1, 31, endpoint=True)
        p = mx.power(1-x, 3)
        n = mx.array([3] * len(x))
        dataset = mx.column_stack([n, x, p])
        FuncData(
    ...
    def test_n_large(self):
        ...
        x = 0.4
        pvals = mx.array([smirnov(n, x) for n in range(400, 1100, 20)])
        dfs = mx.diff(pvals)
        assert_(mx.all(dfs <= 0), msg=f'Not all diffs negative {dfs}')


class TestSmirnovi:
    def test_nan(self):
        assert_(mx.isnan(smirnovi(1, mx.nan)))
    ...
    def test_n_equals_1(self):
        pp = mx.linspace(0, 1, 101, endpoint=True)
        # dataset = mx.array([(1, p, 1-p) for p in pp])
        dataset = mx.column_stack([[1]*len(pp), pp, 1-pp])
        FuncData(
            smirnovi, dataset, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])
        dataset[:, 1] = 1 - dataset[:, 1]
...
class TestSmirnovp:
    def test_nan(self):
        assert_(mx.isnan(_smirnovp(1, mx.nan)))

    def test_basic(self):
        # Check derivative at endpoints
        n1_10 = mx.arange(1, 10)
        dataset0 = mx.column_stack([n1_10,
                                    mx.full_like(n1_10, 0),
                                    mx.full_like(n1_10, -1)])
        FuncData(
            _smirnovp, dataset0, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])

        n2_10 = mx.arange(2, 10)
        dataset1 = mx.column_stack([n2_10,
                                    mx.full_like(n2_10, 1.0),
                                    mx.full_like(n2_10, 0)])
        FuncData(
    ...
    def test_oneminusoneovern(self):
        ...
        xm1 = 1-1.0/n
        pp1 = -n * x**(n-1)
        pp1 -= (1-mx.sign(n-2)**2) * 0.5  # n=2, x=0.5, 1-1/n = 0.5, need to adjust
        dataset1 = mx.column_stack([n, xm1, pp1])
        FuncData(
            _smirnovp, dataset1, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])
    ...
    def test_oneovertwon(self):
        ...
        x = 1.0/2/n
        pp = -(n*x+1) * (1+x)**(n-2)
        dataset0 = mx.column_stack([n, x, pp])
        FuncData(
            _smirnovp, dataset0, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])
    ...
    def test_oneovernclose(self):
        ...
        x = 1.0/n + 2*mx.finfo(float).eps
        pp = -(n*x+1) * (1+x)**(n-2) + 1
        dataset1 = mx.column_stack([n, x, pp])
        FuncData(
            _smirnovp, dataset1, (0, 1), 2, rtol=_rtol
        ).check(dtypes=[int, float, float])
...
class TestKolmogorov:
    def test_nan(self):
        assert_(mx.isnan(kolmogorov(mx.nan)))
    ...
    def test_linspace(self):
        ...
        dataset = mx.column_stack([x, dataset])
        FuncData(kolmogorov, dataset, (0,), 1, rtol=_rtol).check()
        dataset_c = mx.column_stack([x, dataset_c])
        FuncData(_kolmogc, dataset_c, (0,), 1, rtol=_rtol).check()

    def test_linspacei(self):
        p = mx.linspace(0, 1.0, 21, endpoint=True)
        dataset = [mx.inf, 1.3580986393225507, 1.2238478702170823,
        ...
        dataset_c = [0.0000000000000000, 0.5196103791686225, 0.5711732651063401,
                     ...
                     1.2238478702170825, 1.3580986393225509, mx.inf]

        dataset = mx.column_stack([p[1:], dataset[1:]])
        FuncData(kolmogi, dataset, (0,), 1, rtol=_rtol).check()
        dataset_c = mx.column_stack([p[:-1], dataset_c[:-1]])
        FuncData(_kolmogci, dataset_c, (0,), 1, rtol=_rtol).check()

    def test_smallx(self):
        epsilon = 0.1 ** mx.arange(1, 14)
        x = mx.array([0.571173265106, 0.441027698518, 0.374219690278, 0.331392659217,
                      0.300820537459, 0.277539353999, 0.259023494805, 0.243829561254,
                      0.231063086389, 0.220135543236, 0.210641372041, 0.202290283658,
                      0.19487060742])

        dataset = mx.column_stack([x, 1-epsilon])
        FuncData(kolmogorov, dataset, (0,), 1, rtol=_rtol).check()
...
class TestKolmogi:
    ...
    def test_smallpcdf(self):
        ...
        x = mx.array([0.8275735551899077, 0.5345255069097583, 0.4320114038786941,
        ...
        dataset = mx.column_stack([1-epsilon, x])
        FuncData(kolmogi, dataset, (0,), 1, rtol=_rtol).check()

        dataset = mx.column_stack([epsilon, x])
    ...
    def test_smallpsf(self):
        ...
        x = mx.array([0.8275735551899077, 1.3163786275161036, 1.6651092133663343,
    ...
    def test_round_trip(self):
        ...
        p = mx.linspace(0.1, 1.0, 10, endpoint=True)
        dataset = mx.column_stack([p, p])
        FuncData(_k_ki, dataset, (0,), 1, rtol=_rtol).check()


class TestKolmogp:
    def test_nan(self):
        assert_(mx.isnan(_kolmogp(mx.nan)))
