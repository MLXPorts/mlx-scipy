...
class TestLegendre:
    def test_legendre(self):
        ...
        assert_allclose(leg2.c, mx.array([3, 0, -1])/2.0,
                        atol=1.5e-13, rtol=0)
        assert_allclose(leg3.c, mx.array([5, 0, -3, 0])/2.0,
                        atol=1.5e-7, rtol=0)
        assert_allclose(leg4.c, mx.array([35, 0, -30, 0, 3])/8.0,
                        atol=1.5e-7, rtol=0)
        assert_allclose(leg5.c, mx.array([63, 0, -70, 0, 15, 0])/8.0,
                        atol=1.5e-7, rtol=0)
...
class TestLegendreP:
    ...
    def test_ode(self, shape):
        ...
        mx.testing.assert_allclose(err, 0, atol=1e-10)
    ...
    def test_all_ode(self, n_max, x_shape):
        ...
        n = mx.arange(n_max + 1)
        n = mx.expand_dims(n, axis = tuple(range(1, x.ndim + 1)))
...
class TestAssocLegendreP:
    ...
    def test_lpmn(self, shape, n_max, m_max):
        ...
        n = mx.arange(n_max + 1)
        n = mx.expand_dims(n, axis = tuple(range(1, x.ndim + 2)))

        m = mx.concatenate([mx.arange(m_max + 1), mx.arange(-m_max, 0)])
        m = mx.expand_dims(m, axis = (0,) + tuple(range(2, x.ndim + 2)))

        x = mx.expand_dims(x, axis = (0, 1))
        p, p_jac, p_hess = assoc_legendre_p(n, m, x, diff_n=2)

        mx.testing.assert_allclose(p, p_all)
        mx.testing.assert_allclose(p_jac, p_all_jac)
        mx.testing.assert_allclose(p_hess, p_all_hess)
    ...
    def test_ode(self, shape, norm):
        ...
        mx.testing.assert_allclose((1 - x * x) * p_hess,
    ...
    def test_specific(self, shape, norm):
        ...
        mx.testing.assert_allclose(p[0, 0],
            assoc_legendre_p_0_0(x, norm=norm))
        mx.testing.assert_allclose(p[0, 1], 0)
        mx.testing.assert_allclose(p[0, 2], 0)
        mx.testing.assert_allclose(p[0, 3], 0)
        mx.testing.assert_allclose(p[0, 4], 0)
        mx.testing.assert_allclose(p[0, -3], 0)
        mx.testing.assert_allclose(p[0, -2], 0)
        mx.testing.assert_allclose(p[0, -1], 0)

        mx.testing.assert_allclose(p[1, 0],
            assoc_legendre_p_1_0(x, norm=norm))
        mx.testing.assert_allclose(p[1, 1],
            assoc_legendre_p_1_1(x, norm=norm))
        mx.testing.assert_allclose(p[1, 2], 0)
        mx.testing.assert_allclose(p[1, 3], 0)
        mx.testing.assert_allclose(p[1, 4], 0)
        mx.testing.assert_allclose(p[1, -4], 0)
        mx.testing.assert_allclose(p[1, -3], 0)
        mx.testing.assert_allclose(p[1, -2], 0)
        mx.testing.assert_allclose(p[1, -1],
        ...
        mx.testing.assert_allclose(p[2, 0],
            assoc_legendre_p_2_0(x, norm=norm))
        mx.testing.assert_allclose(p[2, 1],
            assoc_legendre_p_2_1(x, norm=norm))
        mx.testing.assert_allclose(p[2, 2],
            assoc_legendre_p_2_2(x, norm=norm))
        mx.testing.assert_allclose(p[2, 3], 0)
        mx.testing.assert_allclose(p[2, 4], 0)
        mx.testing.assert_allclose(p[2, -4], 0)
        mx.testing.assert_allclose(p[2, -3], 0)
        mx.testing.assert_allclose(p[2, -2],
        ...
        mx.testing.assert_allclose(p[3, 0],
            assoc_legendre_p_3_0(x, norm=norm))
        mx.testing.assert_allclose(p[3, 1],
            assoc_legendre_p_3_1(x, norm=norm))
        mx.testing.assert_allclose(p[3, 2],
            assoc_legendre_p_3_2(x, norm=norm))
        mx.testing.assert_allclose(p[3, 3],
            assoc_legendre_p_3_3(x, norm=norm))
        mx.testing.assert_allclose(p[3, 4], 0)
        mx.testing.assert_allclose(p[3, -4], 0)
        mx.testing.assert_allclose(p[3, -3],
        ...
        mx.testing.assert_allclose(p[4, 0],
            assoc_legendre_p_4_0(x, norm=norm))
        mx.testing.assert_allclose(p[4, 1],
            assoc_legendre_p_4_1(x, norm=norm))
        mx.testing.assert_allclose(p[4, 2],
            assoc_legendre_p_4_2(x, norm=norm))
        mx.testing.assert_allclose(p[4, 3],
            assoc_legendre_p_4_3(x, norm=norm))
        mx.testing.assert_allclose(p[4, 4],
        ...
        mx.testing.assert_allclose(p_jac[0, 0],
            assoc_legendre_p_0_0_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[0, 1], 0)
        mx.testing.assert_allclose(p_jac[0, 2], 0)
        mx.testing.assert_allclose(p_jac[0, 3], 0)
        mx.testing.assert_allclose(p_jac[0, 4], 0)
        mx.testing.assert_allclose(p_jac[0, -4], 0)
        mx.testing.assert_allclose(p_jac[0, -3], 0)
        mx.testing.assert_allclose(p_jac[0, -2], 0)
        mx.testing.assert_allclose(p_jac[0, -1], 0)

        mx.testing.assert_allclose(p_jac[1, 0],
            assoc_legendre_p_1_0_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[1, 1],
            assoc_legendre_p_1_1_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[1, 2], 0)
        mx.testing.assert_allclose(p_jac[1, 3], 0)
        mx.testing.assert_allclose(p_jac[1, 4], 0)
        mx.testing.assert_allclose(p_jac[1, -4], 0)
        mx.testing.assert_allclose(p_jac[1, -3], 0)
        mx.testing.assert_allclose(p_jac[1, -2], 0)
        mx.testing.assert_allclose(p_jac[1, -1],
        ...
        mx.testing.assert_allclose(p_jac[2, 0],
            assoc_legendre_p_2_0_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[2, 1],
            assoc_legendre_p_2_1_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[2, 2],
            assoc_legendre_p_2_2_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[2, 3], 0)
        mx.testing.assert_allclose(p_jac[2, 4], 0)
        mx.testing.assert_allclose(p_jac[2, -4], 0)
        mx.testing.assert_allclose(p_jac[2, -3], 0)
        mx.testing.assert_allclose(p_jac[2, -2],
        ...
        mx.testing.assert_allclose(p_jac[3, 0],
            assoc_legendre_p_3_0_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[3, 1],
            assoc_legendre_p_3_1_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[3, 2],
            assoc_legendre_p_3_2_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[3, 3],
            assoc_legendre_p_3_3_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[3, 4], 0)
        mx.testing.assert_allclose(p_jac[3, -4], 0)
        mx.testing.assert_allclose(p_jac[3, -3],
        ...
        mx.testing.assert_allclose(p_jac[4, 0],
            assoc_legendre_p_4_0_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[4, 1],
            assoc_legendre_p_4_1_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[4, 2],
            assoc_legendre_p_4_2_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[4, 3],
            assoc_legendre_p_4_3_jac(x, norm=norm))
        mx.testing.assert_allclose(p_jac[4, 4],
    ...
    def test_all_limits(self, m_max, n_max, x):
        ...
        mx.testing.assert_allclose(p_jac[:, 0],
            pow(x, n + 1) * n * (n + 1) / 2)
        mx.testing.assert_allclose(p_jac[:, 1],
            mx.where(n >= 1, pow(x, n) * mx.inf, 0))
        mx.testing.assert_allclose(p_jac[:, 2],
            mx.where(n >= 2, -pow(x, n + 1) * (n + 2) * (n + 1) * n * (n - 1) / 4, 0))
        mx.testing.assert_allclose(p_jac[:, -2],
            mx.where(n >= 2, -pow(x, n + 1) / 4, 0))
        mx.testing.assert_allclose(p_jac[:, -1],
            mx.where(n >= 1, -pow(x, n) * mx.inf, 0))

        for m in range(3, m_max + 1):
            mx.testing.assert_allclose(p_jac[:, m], 0)
            mx.testing.assert_allclose(p_jac[:, -m], 0)
...
class TestSphLegendreP:
    ...
    def test_specific(self, shape):
        ...
        theta = rng.uniform(-mx.pi, mx.pi, shape)
    ...
    def test_ode(self, shape):
        ...
        mx.testing.assert_allclose(mx.sin(theta) * p_hess, -mx.cos(theta) * p_jac
            - (n * (n + 1) * mx.sin(theta) - m * m / mx.sin(theta)) * p,
            rtol=1e-05, atol=1e-08)
...
class TestLegendreFunctions:
    ...
    def test_clpmn_all_limits(self, m_max, n_max, z):
        ...
        mx.testing.assert_allclose(p_jac[0], pow(z, n + 1) * n * (n + 1) / 2)
        mx.testing.assert_allclose(p_jac[1], mx.where(n >= 1, pow(z, n) * mx.inf, 0))
        mx.testing.assert_allclose(p_jac[2], mx.where(n >= 2,
            -pow(z, n + 1) * (n + 2) * (n + 1) * n * (n - 1) / 4, 0))
        mx.testing.assert_allclose(p_jac[-2], mx.where(n >= 2, -pow(z, n + 1) / 4, 0))
        mx.testing.assert_allclose(p_jac[-1], mx.where(n >= 1, -pow(z, n) * mx.inf, 0))
    ...
    def test_lqn(self):
        lqf = special.lqn(2, .5)
        assert_allclose(lqf, (mx.array([0.5493, -0.7253, -0.8187]),
                              mx.array([1.3333, 1.216, -0.8427])),
                        atol=1.5e-4, rtol=0)
...
def assoc_legendre_p_1_1(z, *, branch_cut=2, norm=False):
    branch_sign = mx.where(branch_cut == 3, mx.where(mx.signbit(mx.real(z)), 1, -1), -1)
    branch_cut_sign = mx.where(branch_cut == 3, -1, 1)
    fac = assoc_legendre_factor(1, 1, norm)

    w = mx.sqrt(mx.where(branch_cut == 3, z * z - 1, 1 - z * z))
...
def assoc_legendre_p_4_4(z, *, branch_cut=2, norm=False):
    ...
    return 105 * fac * mx.square(z * z - 1)

def assoc_legendre_p_4_m4(z, *, branch_cut=2, norm=False):
    fac = assoc_legendre_factor(4, -4, norm)

    return fac * mx.square(z * z - 1) / 384
...
def assoc_legendre_p_1_1_jac_div_z(z, branch_cut=2):
    ...
    out11_div_z = (-branch_sign /
        mx.sqrt(mx.where(branch_cut == 3, z * z - 1, 1 - z * z)))
...
def sph_legendre_factor(n, m):
    return assoc_legendre_factor(n, m, norm=True) / mx.sqrt(2 * mx.pi)

def sph_legendre_p_0_0(theta):
    fac = sph_legendre_factor(0, 0)

    return mx.full_like(theta, fac)

def sph_legendre_p_1_0(theta):
    fac = sph_legendre_factor(1, 0)

    return fac * mx.cos(theta)

def sph_legendre_p_1_1(theta):
    fac = sph_legendre_factor(1, 1)

    return -fac * mx.abs(mx.sin(theta))

def sph_legendre_p_1_m1(theta):
    fac = sph_legendre_factor(1, -1)

    return fac * mx.abs(mx.sin(theta)) / 2

def sph_legendre_p_2_0(theta):
    fac = sph_legendre_factor(2, 0)

    return fac * (3 * mx.square(mx.cos(theta)) - 1) / 2

def sph_legendre_p_2_1(theta):
    fac = sph_legendre_factor(2, 1)

    return -3 * fac * mx.abs(mx.sin(theta)) * mx.cos(theta)

def sph_legendre_p_2_2(theta):
    fac = sph_legendre_factor(2, 2)

    return 3 * fac * (1 - mx.square(mx.cos(theta)))

def sph_legendre_p_2_m2(theta):
    fac = sph_legendre_factor(2, -2)

    return fac * (1 - mx.square(mx.cos(theta))) / 8

def sph_legendre_p_2_m1(theta):
    fac = sph_legendre_factor(2, -1)

    return fac * mx.cos(theta) * mx.abs(mx.sin(theta)) / 2

def sph_legendre_p_3_0(theta):
    fac = sph_legendre_factor(3, 0)

    return (fac * (5 * mx.square(mx.cos(theta)) - 3) *
        mx.cos(theta) / 2)

def sph_legendre_p_3_1(theta):
    fac = sph_legendre_factor(3, 1)

    return (-3 * fac * (5 * mx.square(mx.cos(theta)) - 1) *
        mx.abs(mx.sin(theta)) / 2)

def sph_legendre_p_3_2(theta):
    fac = sph_legendre_factor(3, 2)

    return (-15 * fac * (mx.square(mx.cos(theta)) - 1) *
        mx.cos(theta))

def sph_legendre_p_3_3(theta):
    fac = sph_legendre_factor(3, 3)

    return -15 * fac * mx.power(mx.abs(mx.sin(theta)), 3)

def sph_legendre_p_3_m3(theta):
    fac = sph_legendre_factor(3, -3)

    return fac * mx.power(mx.abs(mx.sin(theta)), 3) / 48

def sph_legendre_p_3_m2(theta):
    fac = sph_legendre_factor(3, -2)

    return (-fac * (mx.square(mx.cos(theta)) - 1) *
        mx.cos(theta) / 8)

def sph_legendre_p_3_m1(theta):
    fac = sph_legendre_factor(3, -1)

    return (fac * (5 * mx.square(mx.cos(theta)) - 1) *
        mx.abs(mx.sin(theta)) / 8)

def sph_legendre_p_4_0(theta):
    fac = sph_legendre_factor(4, 0)

    return (fac * (35 * mx.square(mx.square(mx.cos(theta))) -
        30 * mx.square(mx.cos(theta)) + 3) / 8)

def sph_legendre_p_4_1(theta):
    fac = sph_legendre_factor(4, 1)

    return (-5 * fac * (7 * mx.square(mx.cos(theta)) - 3) *
        mx.cos(theta) * mx.abs(mx.sin(theta)) / 2)

def sph_legendre_p_4_2(theta):
    fac = sph_legendre_factor(4, 2)

    return (-15 * fac * (7 * mx.square(mx.cos(theta)) - 1) *
        (mx.square(mx.cos(theta)) - 1) / 2)

def sph_legendre_p_4_3(theta):
    fac = sph_legendre_factor(4, 3)

    return -105 * fac * mx.power(mx.abs(mx.sin(theta)), 3) * mx.cos(theta)

def sph_legendre_p_4_4(theta):
    fac = sph_legendre_factor(4, 4)

    return 105 * fac * mx.square(mx.square(mx.cos(theta)) - 1)

def sph_legendre_p_4_m4(theta):
    fac = sph_legendre_factor(4, -4)

    return fac * mx.square(mx.square(mx.cos(theta)) - 1) / 384

def sph_legendre_p_4_m3(theta):
    fac = sph_legendre_factor(4, -3)

    return (fac * mx.power(mx.abs(mx.sin(theta)), 3) *
        mx.cos(theta) / 48)

def sph_legendre_p_4_m2(theta):
    fac = sph_legendre_factor(4, -2)

    return (-fac * (7 * mx.square(mx.cos(theta)) - 1) *
        (mx.square(mx.cos(theta)) - 1) / 48)

def sph_legendre_p_4_m1(theta):
    fac = sph_legendre_factor(4, -1)

    return (fac * (7 * mx.square(mx.cos(theta)) - 3) *
        mx.cos(theta) * mx.abs(mx.sin(theta)) / 8)

def sph_legendre_p_0_0_jac(theta):
    return mx.zeros_like(theta)

def sph_legendre_p_1_0_jac(theta):
    fac = sph_legendre_factor(1, 0)

    return -fac * mx.sin(theta)

def sph_legendre_p_1_1_jac(theta):
    fac = sph_legendre_factor(1, 1)

    return -fac * mx.cos(theta) * (2 * mx.heaviside(mx.sin(theta), 1) - 1)

def sph_legendre_p_1_m1_jac(theta):
    fac = sph_legendre_factor(1, -1)

    return fac * mx.cos(theta) * (2 * mx.heaviside(mx.sin(theta), 1) - 1) / 2

def sph_legendre_p_2_0_jac(theta):
    fac = sph_legendre_factor(2, 0)

    return -3 * fac * mx.cos(theta) * mx.sin(theta)

def sph_legendre_p_2_1_jac(theta):
    fac = sph_legendre_factor(2, 1)

    return (3 * fac * (-mx.square(mx.cos(theta)) *
        (2 * mx.heaviside(mx.sin(theta), 1) - 1) +
        mx.abs(mx.sin(theta)) * mx.sin(theta)))

def sph_legendre_p_2_2_jac(theta):
    fac = sph_legendre_factor(2, 2)

    return 6 * fac * mx.sin(theta) * mx.cos(theta)

def sph_legendre_p_2_m2_jac(theta):
    fac = sph_legendre_factor(2, -2)

    return fac * mx.sin(theta) * mx.cos(theta) / 4

def sph_legendre_p_2_m1_jac(theta):
    fac = sph_legendre_factor(2, -1)

    return (-fac * (-mx.square(mx.cos(theta)) *
        (2 * mx.heaviside(mx.sin(theta), 1) - 1) +
        mx.abs(mx.sin(theta)) * mx.sin(theta)) / 2)

def sph_legendre_p_3_0_jac(theta):
    fac = sph_legendre_factor(3, 0)

    return 3 * fac * (1 - 5 * mx.square(mx.cos(theta))) * mx.sin(theta) / 2

def sph_legendre_p_3_1_jac(theta):
    fac = sph_legendre_factor(3, 1)

    return (3 * fac * (11 - 15 * mx.square(mx.cos(theta))) * mx.cos(theta) *
        (2 * mx.heaviside(mx.sin(theta), 1) - 1) / 2)

def sph_legendre_p_3_2_jac(theta):
    fac = sph_legendre_factor(3, 2)

    return 15 * fac * (3 * mx.square(mx.cos(theta)) - 1) * mx.sin(theta)

def sph_legendre_p_3_3_jac(theta):
    fac = sph_legendre_factor(3, 3)

    return -45 * fac * mx.abs(mx.sin(theta)) * mx.sin(theta) * mx.cos(theta)

def sph_legendre_p_3_m3_jac(theta):
    fac = sph_legendre_factor(3, -3)

    return fac * mx.abs(mx.sin(theta)) * mx.sin(theta) * mx.cos(theta) / 16

def sph_legendre_p_3_m2_jac(theta):
    fac = sph_legendre_factor(3, -2)

    return fac * (3 * mx.square(mx.cos(theta)) - 1) * mx.sin(theta) / 8

def sph_legendre_p_3_m1_jac(theta):
    fac = sph_legendre_factor(3, -1)

    return (-fac * (11 - 15 * mx.square(mx.cos(theta))) *
        mx.cos(theta) *
        (2 * mx.heaviside(mx.sin(theta), 1) - 1) / 8)

def sph_legendre_p_4_0_jac(theta):
    fac = sph_legendre_factor(4, 0)

    return (-5 * fac * (7 * mx.square(mx.cos(theta)) - 3) *
        mx.sin(theta) * mx.cos(theta) / 2)

def sph_legendre_p_4_1_jac(theta):
    fac = sph_legendre_factor(4, 1)

    return (5 * fac * (-3 + 27 * mx.square(mx.cos(theta)) -
        28 * mx.square(mx.square(mx.cos(theta)))) *
        (2 * mx.heaviside(mx.sin(theta), 1) - 1) / 2)

def sph_legendre_p_4_2_jac(theta):
    fac = sph_legendre_factor(4, 2)

    return (30 * fac * (7 * mx.square(mx.cos(theta)) - 4) *
        mx.sin(theta) * mx.cos(theta))

def sph_legendre_p_4_3_jac(theta):
    fac = sph_legendre_factor(4, 3)

    return (-105 * fac * (4 * mx.square(mx.cos(theta)) - 1) *
        mx.abs(mx.sin(theta)) * mx.sin(theta))

def sph_legendre_p_4_4_jac(theta):
    fac = sph_legendre_factor(4, 4)

    return (-420 * fac * (mx.square(mx.cos(theta)) - 1) *
        mx.sin(theta) * mx.cos(theta))
...
def sph_legendre_p_4_m4_jac(theta):
    ...
    return (-fac * (mx.square(mx.cos(theta)) - 1) *
        mx.sin(theta) * mx.cos(theta) / 96)

def sph_legendre_p_4_m3_jac(theta):
    fac = sph_legendre_factor(4, -3)

    return (fac * (4 * mx.square(mx.cos(theta)) - 1) *
        mx.abs(mx.sin(theta)) * mx.sin(theta) / 48)

def sph_legendre_p_4_m2_jac(theta):
    fac = sph_legendre_factor(4, -2)

    return (fac * (7 * mx.square(mx.cos(theta)) - 4) * mx.sin(theta) *
        mx.cos(theta) / 12)

def sph_legendre_p_4_m1_jac(theta):
    fac = sph_legendre_factor(4, -1)

    return (-fac * (-3 + 27 * mx.square(mx.cos(theta)) -
        28 * mx.square(mx.square(mx.cos(theta)))) *
        (2 * mx.heaviside(mx.sin(theta), 1) - 1) / 8)
