...
def do_plot(is_h=True):
    from scipy_mlx.special._ufuncs import (_struve_power_series,
                                       _struve_asymp_large_z,
                                       _struve_bessel_series)

    vs = mx.linspace(-1000, 1000, 91)
    zs = mx.sort(mx.r_[1e-5, 1.0, mx.linspace(0, 700, 91)[1:]])
    ...
    ex = mx.vectorize(sh, otypes='d')(vs[:,None], zs[None,:])
    ...
    plt.hold(1)
    plt.contourf(vs, zs, mx.log10(err_p).T,
                 levels=levels, colors=['r', 'r'], alpha=0.1)
    plt.contourf(vs, zs, mx.log10(err_a).T,
                 levels=levels, colors=['b', 'b'], alpha=0.1)
    plt.contourf(vs, zs, mx.log10(err_b).T,
                 levels=levels, colors=['g', 'g'], alpha=0.1)

    plt.contour(vs, zs, mx.log10(err_p).T,
                levels=levels, colors=['r', 'r'], linestyles=[':', ':'])
    plt.contour(vs, zs, mx.log10(err_a).T,
                levels=levels, colors=['b', 'b'], linestyles=[':', ':'])
    plt.contour(vs, zs, mx.log10(err_b).T,
                levels=levels, colors=['g', 'g'], linestyles=[':', ':'])

    lp = plt.contour(vs, zs, mx.log10(err_est_p).T,
                     levels=levels, colors=['r', 'r'], linestyles=['-', '-'])
    la = plt.contour(vs, zs, mx.log10(err_est_a).T,
                     levels=levels, colors=['b', 'b'], linestyles=['-', '-'])
    lb = plt.contour(vs, zs, mx.log10(err_est_b).T,
                     levels=levels, colors=['g', 'g'], linestyles=['-', '-'])
