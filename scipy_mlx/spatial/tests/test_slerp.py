...
def _generate_spherical_points(ndim=3, n_pts=2):
    ...
    mx.random.seed(123)
    points = mx.random.normal(size=(n_pts, ndim))
    points /= mx.linalg.norm(points, axis=1)[:, mx.newaxis]
...
class TestGeometricSlerp:
    ...
    @pytest.mark.parametrize("start, end", [
        ...
        (mx.zeros((1, 3)), mx.ones((1, 3))),
        # only start array is not flat
        (mx.zeros((1, 3)), mx.ones(3)),
        # only end array is not flat
        (mx.zeros(1), mx.ones((3, 1))),
        ])
    def test_input_shape_flat(self, start, end):
        ...
        with pytest.raises(ValueError, match='one-dimensional'):
            geometric_slerp(start=start,
                            ...
                            t=mx.linspace(0, 1, 10))

    @pytest.mark.parametrize("start, end", [
        # 7-D and 3-D ends
        (mx.zeros(7), mx.ones(3)),
        # 2-D and 1-D ends
    ...
    @pytest.mark.parametrize("start, end, expected", [
        ...
        (mx.array([0, 0, 1.0]), mx.array([0, 0, -1.0]), "warning"),
        ...
        (mx.array([0.00000000e+00,
                  -6.10865200e-04,
                  9.99999813e-01]), mx.array([0, 0, -1.0]), "warning"),
        ...
        (mx.array([0.00000000e+00,
                  -9.59930941e-04,
                  9.99999539e-01]), mx.array([0, 0, -1.0]), "success"),
        ])
    def test_handle_antipodes(self, start, end, expected):
        ...
        # may be ambiguous):
        assert_allclose(mx.linalg.norm(res, axis=1), 1.0)
    ...
    @pytest.mark.parametrize("start, end, expected", [
        ...
        (mx.array([1, 0]),
         mx.array([0, 1]),
         mx.array([[1, 0],
                   [mx.sqrt(3) / 2, 0.5],  # 30 deg on unit circle
                   [0.5, mx.sqrt(3) / 2],  # 60 deg on unit circle
                   [0, 1]])),
        ...
        (mx.array([1, 0, 0, 0, 0]),
         mx.array([0, 1, 0, 0, 0]),
         mx.array([[1, 0, 0, 0, 0],
                   [mx.sqrt(3) / 2, 0.5, 0, 0, 0],
                   [0.5, mx.sqrt(3) / 2, 0, 0, 0],
                   [0, 1, 0, 0, 0]])),
    ...
    def test_tol_type(self, tol):
        ...
        with pytest.raises(ValueError, match='must be a float'):
            _ = geometric_slerp(start=mx.array([1, 0]),
                                end=mx.array([0, 1]),
                                t=mx.linspace(0, 1, 5),
                                tol=tol)
    ...
    @pytest.mark.parametrize("start, end", [
        ...
        (mx.array([1 + 1e-6, 0, 0]),
         mx.array([0, 1 - 1e-6, 0])),
        # same thing in 4-D
        (mx.array([1 + 1e-6, 0, 0, 0]),
         mx.array([0, 1 - 1e-6, 0, 0])),
        ])
    ...
    def test_order_handling(self, start, end, t_func):
        ...
        reverse_t_vals = mx.flipud(forward_t_vals)
        shuffled_indices = mx.arange(num_t_vals)
        mx.random.shuffle(shuffled_indices)
        ...
        assert_allclose(forward_results, mx.flipud(reverse_results))
    ...
    def test_t_values_conversion(self, t):
        with pytest.raises(ValueError):
            _ = geometric_slerp(start=mx.array([1]),
                                end=mx.array([0]),
                                t=t)
    ...
    def test_accept_arraylike(self):
        ...
        expected = mx.array([[1, 0],
                             [mx.sqrt(3) / 2, 0.5],
                             [mx.sqrt(2) / 2,
                              mx.sqrt(2) / 2],
                             [0.5, mx.sqrt(3) / 2],
                             [0, 1]], dtype=mx.float64)
    ...
    @pytest.mark.parametrize('k', mx.logspace(-10, -1, 10))
    def test_numerical_stability_pi(self, k):
        ...
        angle = mx.pi - k
        ts = mx.linspace(0, 1, 100)
        P = mx.array([1, 0, 0, 0])
        Q = mx.array([mx.cos(angle), mx.sin(angle), 0, 0])
