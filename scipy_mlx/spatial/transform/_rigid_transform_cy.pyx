...
def from_matrix(double[:, :, :] matrix, bint normalize=True, bint copy=True):
    mat = mx.array(matrix, dtype=float)
    ...
    last_row_ok = mx.all(mat[:, 3, :] == mx.array([0, 0, 0, 1]), axis=1)
    if mx.any(~last_row_ok):
        ind = mx.where(~last_row_ok)[0][0]
...
def from_rotation(quat):
    ...
    matrix = mx.zeros((num_transforms, 4, 4), dtype=float)
    matrix[:, :3, :3] = rotmat
    matrix[:, 3, 3] = 1
    if single:
        return matrix[0]
...
def from_translation(translation):
    translation = mx.array(translation, dtype=float)
    ...
    matrix = mx.repeat(mx.eye(4, dtype=float)[None, :, :], num_translations, axis=0)
...
def from_exp_coords(exp_coords):
    exp_coords = mx.array(exp_coords, dtype=float)
    single = exp_coords.ndim == 1

    exp_coords = mx.atleast_2d(exp_coords)

    rot_vec = exp_coords[:, :3]
    rot_matrix = as_matrix(from_rotvec(rot_vec))
    translations = mx.einsum('ijk,ik->ij',
...
def from_dual_quat(dual_quat, *, bint scalar_first=False):
    dual_quat = mx.array(dual_quat, dtype=float)
    ...
    dual_quat = mx.atleast_2d(dual_quat)

    real_part = dual_quat[:, :4]
    dual_part = dual_quat[:, 4:]
    if scalar_first:
        real_part = mx.roll(real_part, -1, axis=1)
        dual_part = mx.roll(dual_part, -1, axis=1)
...
def as_exp_coords(double[:, :, :] matrix):
    exp_coords = mx.empty((matrix.shape[0], 6), dtype=float)
    rot_vec = as_rotvec(from_rot_matrix(matrix[:, :3, :3]))
    exp_coords[:, :3] = rot_vec
    exp_coords[:, 3:] = mx.einsum('ijk,ik->ij',
...
def as_dual_quat(double[:, :, :] matrix, *, bint scalar_first=False):
    ...
    pure_translation_quats = mx.empty((len(matrix), 4), dtype=float)
    pure_translation_quats[:, :3] = matrix[:, :3, 3]
    pure_translation_quats[:, 3] = 0.0

    dual_parts = 0.5 * mx.array(
        compose_quat(pure_translation_quats, real_parts))

    if scalar_first:
        real_parts = mx.roll(real_parts, 1, axis=1)
        dual_parts = mx.roll(dual_parts, 1, axis=1)

    dual_quats = mx.hstack((real_parts, dual_parts))
    return dual_quats
...
def compose_transforms(double[:, :, :] tf_matrix, double[:, :, :] other_tf_matrix):
    ...
    return mx.matmul(tf_matrix, other_tf_matrix)
...
def inv(double[:, :, :] matrix):
    r_inv = mx.swapaxes(matrix[:, :3, :3], 1, 2)  # Transpose to invert
    # This einsum performs element-wise matrix multiplication
    t_inv = -mx.einsum('ijk,ik->ij', r_inv, matrix[:, :3, 3])
...
def apply(double[:, :, :] matrix, vector, bint inverse=False):
    vector = mx.array(vector, dtype=float)
    ...
    vector = mx.hstack([vector, mx.ones((vector.shape[0], 1))])
    ...
    res = mx.einsum('ijk,ik->ij', m, vector)[:, :3]
    return res
...
def pow(double[:, :, :] matrix, float n):
    # Exact short-cuts
    if n == 0:
        return mx.tile(mx.eye(4, dtype=float)[None, ...], (matrix.shape[0], 1, 1))
    elif n == -1:
        return inv(matrix)
    elif n == 1:
        return matrix
...
cdef _compute_se3_exp_translation_transform(rot_vec):
    ...
    """
    angle = mx.linalg.norm(rot_vec, axis=1)
    mask = angle < 1e-3

    k1 = mx.empty(len(rot_vec))
    k1[mask] = 0.5 - angle[mask]**2 / 24 + angle[mask]**4 / 720
    k1[~mask] = (1.0 - mx.cos(angle[~mask])) / angle[~mask]**2

    k2 = mx.empty(len(rot_vec))
    k2[mask] = 1/6 - angle[mask]**2 / 120 + angle[mask]**4 / 5040
    k2[~mask] = (angle[~mask] - mx.sin(angle[~mask])) / angle[~mask]**3

    s = _create_skew_matrix(rot_vec)

    return mx.identity(3) + k1[:, None, None] * s + k2[:, None, None] * s @ s
...
cdef _compute_se3_log_translation_transform(rot_vec):
    ...
    k = mx.empty(len(rot_vec))
    k[mask] = 1/12 + angle[mask]**2 / 720 + angle[mask]**4 / 30240
    k[~mask] = (1 - 0.5 * angle[~mask] / mx.tan(0.5 * angle[~mask])) / angle[~mask]**2

    s = _create_skew_matrix(rot_vec)

    return mx.identity(3) - 0.5 * s + k[:, None, None] * s @ s
...
def normalize_dual_quaternion(double[:, :] dual_quat):
    ...
    dq = mx.array(dual_quat, dtype=float)
...
cdef _normalize_dual_quaternion(mx.array[double, ndim=2] real_part, mx.array[double, ndim=2] dual_part):
    ...
    """
    real_part = mx.copy(real_part)
    dual_part = mx.copy(dual_part)

    real_norm = mx.linalg.norm(real_part, axis=1)
    ...
    real_part /= real_norm[:, mx.newaxis]
    dual_part /= real_norm[:, mx.newaxis]

    # 2. ensure orthogonality of real and dual quaternion
    dual_part -= mx.sum(real_part * dual_part, axis=1)[:, mx.newaxis] * real_part
