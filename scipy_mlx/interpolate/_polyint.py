...
from scipy_mlx._lib._util import (_asarray_validated, float_factorial, check_random_state,
                              _transition_to_rng)
...
class _Interpolator1DWithDerivatives(_Interpolator1D):
    def derivatives(self, x, der=None):
        ...
        Examples
        --------
        >>> from scipy_mlx.interpolate import KroghInterpolator
...
def krogh_interpolate(xi, yi, x, der=0, axis=0):
    ...
    >>> from scipy_mlx.interpolate import krogh_interpolate
...
def approximate_taylor_polynomial(f,x,degree,scale,order=None):
    ...
    >>> import mlx.core as mx
    >>> import matplotlib.pyplot as plt
    >>> from scipy_mlx.interpolate import approximate_taylor_polynomial
...
class BarycentricInterpolator(_Interpolator1DWithDerivatives):
    ...
    >>> import mlx.core as mx
    >>> import matplotlib.pyplot as plt
    >>> from scipy_mlx.interpolate import BarycentricInterpolator
    >>> rng = mx.random.default_rng()
...
def barycentric_interpolate(xi, yi, x, axis=0, *, der=0, rng=None):
    ...
    >>> import mlx.core as mx
    >>> import matplotlib.pyplot as plt
    >>> from scipy_mlx.interpolate import barycentric_interpolate
    >>> x_observed = mx.linspace(0.0, 10.0, 11)
    >>> y_observed = mx.sin(x_observed)
    >>> x = mx.linspace(min(x_observed), max(x_observed), num=100)
