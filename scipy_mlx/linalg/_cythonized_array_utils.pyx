...
from scipy_mlx.linalg._cythonized_array_utils cimport (
    lapack_t,
...
def bandwidth(a):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import bandwidth
...
def issymmetric(a, atol=None, rtol=None):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import issymmetric
...
def ishermitian(a, atol=None, rtol=None):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import ishermitian
    >>> A = mx.arange(9).reshape(3, 3)
    >>> A = A + A.T
    >>> ishermitian(A)
    True
