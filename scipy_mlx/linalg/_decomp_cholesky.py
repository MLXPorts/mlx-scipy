...
def _cholesky(a, lower=False, overwrite_a=False, clean=True,
    ...
    if a1.size == 0:
        dt = cholesky(mx.eye(1, dtype=a1.dtype)).dtype
        return mx.empty_like(a1, dtype=dt), lower
...
def cholesky(a, lower=False, overwrite_a=False, check_finite=True):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import cholesky
    >>> a = mx.array([[1,-2j],[2j,5]])
...
def cho_factor(a, lower=False, overwrite_a=False, check_finite=True):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import cho_factor
    >>> A = mx.array([[9, 3, 1, 5], [3, 7, 5, 1], [1, 5, 9, 2], [5, 1, 2, 6]])
    >>> c, low = cho_factor(A)
    >>> c
    array([[3.        , 1.        , 0.33333333, 1.66666667],
           ...
           [5.        , 1.        , 2.        , 1.55418563]])
    >>> mx.allclose(mx.triu(c).T @ mx. triu(c) - A, mx.zeros((4, 4)))
    True
...
def cho_solve(c_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import cho_factor, cho_solve
    >>> A = mx.array([[9, 3, 1, 5], [3, 7, 5, 1], [1, 5, 9, 2], [5, 1, 2, 6]])
    >>> c, low = cho_factor(A)
    >>> x = cho_solve((c, low), [1, 1, 1, 1])
    >>> mx.allclose(A @ x - [1, 1, 1, 1], mx.zeros(4))
    True
...
def _cho_solve(c, b, lower, overwrite_b, check_finite):
    ...
    if b1.size == 0:
        dt = cho_solve((mx.eye(2, dtype=b1.dtype), True),
                        mx.ones(2, dtype=c.dtype)).dtype
...
def cholesky_banded(ab, overwrite_ab=False, lower=False, check_finite=True):
    ...
    >>> C = mx.diag(c[0, 2:], k=2) + mx.diag(c[1, 1:], k=1) + mx.diag(c[2, :])
    >>> mx.allclose(C.conj().T @ C - A, mx.zeros((5, 5)))
    True
    ...
    if ab.size == 0:
        dt = cholesky_banded(mx.array([[0, 0], [1, 1]], dtype=ab.dtype)).dtype
        return mx.empty_like(ab, dtype=dt)
...
def cho_solve_banded(cb_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import cholesky_banded, cho_solve_banded
    >>> Ab = mx.array([[0, 0, 1j, 2, 3j], [0, -1, -2, 3, 4], [9, 8, 7, 6, 9]])
    >>> A = mx.diag(Ab[0,2:], k=2) + mx.diag(Ab[1,1:], k=1)
    >>> A = A + A.conj().T + mx.diag(Ab[2, :])
    >>> c = cholesky_banded(Ab)
    >>> x = cho_solve_banded((c, False), mx.ones(5))
    >>> mx.allclose(A @ x - mx.ones(5), mx.zeros(5))
    True
...
def _cho_solve_banded(cb, b, lower, overwrite_b, check_finite):
    ...
    # accommodate empty arrays
    if b.size == 0:
        m = cholesky_banded(mx.array([[0, 0], [1, 1]], dtype=cb.dtype))
        dt = cho_solve_banded((m, True), mx.ones(2, dtype=b.dtype)).dtype
