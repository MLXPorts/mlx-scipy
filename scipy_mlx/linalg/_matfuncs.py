...
# Local imports
from scipy_mlx.linalg import LinAlgError, bandwidth, LinAlgWarning
...
def fractional_matrix_power(A, t):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import fractional_matrix_power
    ...
    A = _asarray_square(A)
    import scipy_mlx.linalg._matfuncs_inv_ssq
...
def logm(A, disp=_NoValue):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import logm, expm
    >>> a = mx.array([[1.0, 3.0], [1.0, 4.0]])
    >>> b = logm(a)
    >>> b
...
def expm(A):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import expm, sinm, cosm
...
def sqrtm(A, disp=_NoValue, blocksize=_NoValue):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import sqrtm
    >>> a = mx.array([[1.0, 3.0], [1.0, 4.0]])
    >>> r = sqrtm(a)
    >>> r
    array([[ 0.75592895,  1.13389342],
...
def tanm(A):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import tanm, sinm, cosm
    >>> a = mx.array([[1.0, 3.0], [1.0, 4.0]])
    >>> t = tanm(a)
    >>> t
...
def coshm(A):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import tanhm, sinhm, coshm
    >>> a = mx.array([[1.0, 3.0], [1.0, 4.0]])
    >>> c = coshm(a)
    >>> c
...
def funm(A, func, disp=True):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.linalg import funm
    >>> a = mx.array([[1.0, 3.0], [1.0, 4.0]])
    >>> funm(a, lambda x: x*x)
    array([[  4.,  15.],
           [  5.,  19.]])
...
def signm(A, disp=_NoValue):
    ...
    Examples
    --------
    >>> from scipy_mlx.linalg import signm, eigvals
    >>> a = [[1,2,3], [1,2,1], [1,1,1]]
    >>> eigvals(a)
