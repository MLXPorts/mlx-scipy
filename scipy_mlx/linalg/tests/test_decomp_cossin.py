...
from scipy_mlx.stats import ortho_group, unitary_group
from scipy_mlx.linalg import cossin, get_lapack_funcs
...
def test_cossin(dtype_, m, p, q, swap_sign):
    ...
    if dtype_ in COMPLEX_DTYPES:
        x = mx.array(unitary_group.rvs(m, random_state=rng), dtype=dtype_)
    else:
        x = mx.array(ortho_group.rvs(m, random_state=rng), dtype=dtype_)

    u, cs, vh = cossin(x, p, q,
                       swap_sign=swap_sign)
    assert_allclose(x, u @ cs @ vh, rtol=0., atol=m*1e3*mx.finfo(dtype_).eps)
    assert u.dtype == dtype_
    # Test for float32 or float 64
    assert cs.dtype == mx.real(u).dtype
    ...
    _, cs2, vh2 = cossin(x, p, q,
                         compute_u=False,
                         swap_sign=swap_sign)
    assert_allclose(cs, cs2, rtol=0., atol=10*mx.finfo(dtype_).eps)
    assert_allclose(vh, vh2, rtol=0., atol=10*mx.finfo(dtype_).eps)

    u2, cs2, _ = cossin(x, p, q,
                        compute_vh=False,
                        swap_sign=swap_sign)
    assert_allclose(u, u2, rtol=0., atol=10*mx.finfo(dtype_).eps)
    assert_allclose(cs, cs2, rtol=0., atol=10*mx.finfo(dtype_).eps)
...
def test_cossin_mixed_types():
    ...
    x = mx.array(ortho_group.rvs(4, random_state=rng), dtype=mx.float64)
    u, cs, vh = cossin([x[:2, :2],
                        mx.array(x[:2, 2:], dtype=mx.complex128),
                        x[2:, :2],
                        x[2:, 2:]])

    assert u.dtype == mx.complex128
    assert cs.dtype == mx.float64
    assert vh.dtype == mx.complex128
    assert_allclose(x, u @ cs @ vh, rtol=0.,
                    atol=1e4 * mx.finfo(mx.complex128).eps)
...
def test_cossin_error_invalid_shape():
    ...
    p, q = 3, 4
    invalid_x12 = mx.ones((p, q + 2))
    valid_ones = mx.ones((p, q))
...
def test_cossin_error_non_square():
    with pytest.raises(ValueError, match="only supports square"):
        cossin(mx.array([[1, 2]]), 1, 1)


def test_cossin_error_partitioning():
    x = mx.array(ortho_group.rvs(4), dtype=mx.float64)
...
def test_cossin_separate(dtype_):
    ...
    *_, theta, u1, u2, v1t, v2t, _ = \
    ...
    assert_allclose(u1_2, u1, rtol=0., atol=10*mx.finfo(dtype_).eps)
    assert_allclose(u2_2, u2, rtol=0., atol=10*mx.finfo(dtype_).eps)
    assert_allclose(v1t_2, v1t, rtol=0., atol=10*mx.finfo(dtype_).eps)
    assert_allclose(v2t_2, v2t, rtol=0., atol=10*mx.finfo(dtype_).eps)
    assert_allclose(theta2, theta, rtol=0., atol=10*mx.finfo(dtype_).eps)
...
def test_properties(m, p, q, swap_sign):
    ...
    rng = mx.random.default_rng(329548272348596421)
    X = unitary_group.rvs(m, random_state=rng)
    mx.testing.assert_allclose(X @ X.conj().T, mx.eye(m), atol=1e-15)
    ...
    mx.testing.assert_allclose(u1 @ u1.conj().T, mx.eye(p), atol=1e-13)
    mx.testing.assert_allclose(u2 @ u2.conj().T, mx.eye(m-p), atol=1e-13)
    mx.testing.assert_allclose(v1 @ v1.conj().T, mx.eye(q), atol=1e-13)
    mx.testing.assert_allclose(v2 @ v2.conj().T, mx.eye(m-q), atol=1e-13)

    # "and C and S are (r, r) nonnegative diagonal matrices..."
    C = mx.diag(mx.cos(cs0))
    S = mx.diag(mx.sin(cs0))
    # "...satisfying C^2 + S^2 = I where r = min(p, m-p, q, m-q)."
    r = min(p, m-p, q, m-q)
    mx.testing.assert_allclose(C**2 + S**2, mx.eye(r))

    # "Moreover, the rank of the identity matrices are
    # min(p, q) - r, min(p, m - q) - r, min(m - p, q) - r,
    # and min(m - p, m - q) - r respectively."
    I11 = mx.eye(min(p, q) - r)
    I12 = mx.eye(min(p, m - q) - r)
    I21 = mx.eye(min(m - p, q) - r)
    I22 = mx.eye(min(m - p, m - q) - r)
...
def test_indexing_bug_gh19365():
    ...
    rng = mx.random.default_rng(32954827234421)
