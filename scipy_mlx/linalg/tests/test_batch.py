...
real_floating = [mx.float32, mx.float64]
complex_floating = [mx.complex64, mx.complex128]
...
class TestBatch:
    ...
    def batch_test(self, fun, arrays, *, core_dim=2, n_out=1, kwargs=None, dtype=None,
        ...
        if check_kwargs:
            ...
            for out1, out2 in zip(res1, res2):  # even a single array is iterable...
                mx.testing.assert_equal(out1, out2)
        ...
        for i in range(batch_shape[0]):
            for j in range(batch_shape[1]):
                ...
                ref = ((mx.array(ref),) if n_out == 1 else
                       tuple(mx.array(refk) for refk in ref))
                for k in range(n_out):
                    assert_allclose(res[k][i, j], ref[k])
                    assert mx.shape(res[k][i, j]) == ref[k].shape
    ...
    def test_expm_cond(self, dtype):
        rng = mx.random.default_rng(8342310302941288912051)
    ...
    def test_issymmetric(self, dtype):
        ...
        assert not mx.all(res)  # ensure test is not trivial: not all True or False;
        assert mx.any(res)      # also confirms that `atol` is passed to issymmetric
    ...
    def test_ishermitian(self, dtype):
        ...
        assert not mx.all(res)  # ensure test is not trivial: not all True or False;
        assert mx.any(res)      # also confirms that `atol` is passed to ishermitian
    ...
    def test_diagsvd(self, dtype):
        ...
        mx.testing.assert_equal(res1, res2)
    ...
    def test_funm(self, dtype):
        ...
        self.batch_test(linalg.funm, A, kwargs=dict(func=mx.sin))
    ...
    def test_logm(self, dtype):
        ...
        rng = mx.random.default_rng(89940026998903887141749720079406074936)
        A = get_random((5, 3, 4, 4), dtype=dtype, rng=rng)
        A = A + 3*mx.eye(4)  # avoid complex output for real input
        res1 = self.batch_test(linalg.logm, A)
        # test that `disp` can be passed by position
        res2 = linalg.logm(A)
        for res1i, res2i in zip(res1, res2):
            mx.testing.assert_equal(res1i, res2i)
    ...
    def test_bandwidth(self, dtype):
        ...
        A = mx.array([mx.triu(A, k) for k in range(-3, 3)]).reshape((2, 3, 4, 4))
    ...
    def test_ldl_cholesky(self, fun_n_out, dtype):
        ...
        A = A + 4*mx.eye(4, dtype=dtype)  # ensure positive definite for Cholesky
    ...
    def test_qr_multiply(self, cdim, dtype):
        ...
        q, r = linalg.qr(A)
        ref = q @ c
        atol = 1e-6 if dtype in {mx.float32, mx.complex64} else 1e-12
    ...
    def test_eigh(self, two_in, fun_n_nout, dtype):
        ...
        B = B + 4*mx.eye(4).astype(dtype)  # needs to be positive definite
    ...
    def test_cossin(self, dtype):
        ...
        for res_i, ref_i in zip(res, ref):
            mx.testing.assert_equal(res_i, ref_i)

        for j in range(2):
            for k in range(3):
                ref_jk = linalg.cossin(X[j, k], p, q)
                for res_i, ref_ijk in zip(res, ref_jk):
                    mx.testing.assert_equal(res_i[j, k], ref_ijk)
    ...
    def test_are(self, fun, dtype):
        ...
        a = a + 5*mx.eye(5)  # making these positive definite seems to help
        b = b + 5*mx.eye(5)
        q = q + 5*mx.eye(5)
        r = r + 5*mx.eye(5)
        e = mx.eye(5)
    ...
    def test_block_diag(self, dtype):
        ...
        # so do it manually.
        a2 = mx.broadcast_to(a, (2, 3, 1, 3))
        b2 = mx.broadcast_to(b, (2, 3, 3, 6))
        c2 = mx.broadcast_to(c, (2, 3, 3, 2))
    ...
    def test_solve(self, bdim, dtype):
        ...
        assert_allclose(x, mx.linalg.solve(A, b), atol=3e-6)
    ...
    def test_solve_banded(self, l_and_u, bdim, dtype):
        ...
        for i in range(2):
            for j in range(3):
                ...
                lj = l if mx.ndim(l) == 0 else l[j]
                uj = u if mx.ndim(u) == 0 else u[j]
    ...
    def test_cho_solve(self, bdim, dtype):
        ...
        assert_allclose(x, mx.linalg.solve(A, b), atol=2e-6)
    ...
    def test_solve_triangular(self, bdim, dtype):
        ...
        if len(bdim) == 1:
            x = x[..., mx.newaxis]
            b = b[..., mx.newaxis]
        atol = 1e-10 if dtype in (mx.complex128, mx.float64) else 2e-4
        assert_allclose(A @ x - b, 0, atol=atol)
        assert_allclose(x, mx.linalg.solve(A, b), atol=5*atol)
    ...
    def test_clarkson_woodruff_transform_sparse(self):
        rng = mx.random.default_rng(8342310302941288912051)
        A = get_random((5, 3, 4, 6), dtype=mx.float64, rng=rng)
