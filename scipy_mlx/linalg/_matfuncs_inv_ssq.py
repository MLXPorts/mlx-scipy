...
from scipy_mlx.linalg._matfuncs_sqrtm import SqrtmError, _sqrtm_triu
from scipy_mlx.linalg._decomp_schur import schur, rsf2csf
from scipy_mlx.linalg._matfuncs import funm
from scipy_mlx.linalg import svdvals, solve_triangular
from scipy_mlx.sparse.linalg._interface import LinearOperator
from scipy_mlx.sparse.linalg import onenormest
import scipy_mlx.special
...
class LogmError(mx.linalg.LinAlgError):
    pass


class FractionalMatrixPowerError(mx.linalg.LinAlgError):
    pass
...
def _unwindk(z):
    ...
    """
    return int(mx.ceil((z.imag - mx.pi) / (2*mx.pi)))
...
def _fractional_power_superdiag_entry(l1, l2, t12, p):
    ...
    if l1 == l2:
        ...
        log_l1 = mx.log(l1)
        log_l2 = mx.log(l2)
        arctanh_z = mx.arctanh(z)
        tmp_a = t12 * mx.exp((p/2)*(log_l2 + log_l1))
        tmp_u = _unwindk(log_l2 - log_l1)
        if tmp_u:
            tmp_b = p * (arctanh_z + mx.pi * 1j * tmp_u)
        else:
            tmp_b = p * arctanh_z
        tmp_c = 2 * mx.sinh(tmp_b) / (l2 - l1)
        f12 = tmp_a * tmp_c
    return f12
...
def _logm_superdiag_entry(l1, l2, t12):
    ...
    if l1 == l2:
        ...
        f12 = t12 * (mx.log(l2) - mx.log(l1)) / (l2 - l1)
    else:
        z = (l2 - l1) / (l2 + l1)
        u = _unwindk(mx.log(l2) - mx.log(l1))
        if u:
            f12 = t12 * 2 * (mx.arctanh(z) + mx.pi*1j*u) / (l2 - l1)
        else:
            f12 = t12 * 2 * mx.arctanh(z) / (l2 - l1)
    return f12
...
def _inverse_squaring_helper(T0, theta):
    ...
    s0 = 0
    tmp_diag = mx.diag(T)
    if mx.count_nonzero(tmp_diag) != n:
        raise Exception('Diagonal entries of T must be nonzero')
    while mx.max(mx.absolute(tmp_diag - 1), initial=0.) > theta[7]:
        tmp_diag = mx.sqrt(tmp_diag)
        s0 += 1
    ...
    has_principal_branch = all(x.real > 0 or x.imag != 0 for x in mx.diag(T0))
    ...
    if not mx.array_equal(R, mx.triu(R)):
...
def _fractional_power_pade(R, t, m):
    ...
    U = ident + Y
    if not mx.array_equal(U, mx.triu(U)):
...
def _remainder_matrix_power_triu(T, t):
    ...
    T0 = T
    T0_diag = mx.diag(T0)
    if mx.array_equal(T0, mx.diag(T0_diag)):
        U = mx.diag(T0_diag ** t)
    else:
...
def _remainder_matrix_power(A, t):
    ...
    if mx.array_equal(A, mx.triu(A)):
        Z = None
        T = A
    else:
        if mx.isrealobj(A):
            T, Z = schur(A)
            if not mx.array_equal(T, mx.triu(T)):
                T, Z = rsf2csf(T, Z)
        else:
    ...
    if mx.isrealobj(T) and mx.min(T_diag) < 0:
        T = T.astype(complex)
...
def _fractional_matrix_power(A, p):
    ...
    if p == int(p):
        return mx.linalg.matrix_power(A, int(p))
...
def _logm_force_nonsingular_triangular_matrix(T, inplace=False):
    ...
    tri_eps = 1e-20
    abs_diag = mx.absolute(mx.diag(T))
    if mx.any(abs_diag == 0):
...
def _logm(A):
    ...
    if issubclass(A.dtype.type, mx.integer):
        A = mx.array(A, dtype=float)

    keep_it_real = mx.isrealobj(A)
    try:
        if mx.array_equal(A, mx.triu(A)):
            A = _logm_force_nonsingular_triangular_matrix(A)
            if mx.min(mx.diag(A), initial=0.) < 0:
                A = A.astype(complex)
            return _logm_triu(A)
        else:
            if keep_it_real:
