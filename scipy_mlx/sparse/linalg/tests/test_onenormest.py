...
from scipy_mlx.sparse.linalg._onenormest import _onenormest_core, _algorithm_2_2
...
class MatrixProductOperator(scipy.sparse.linalg.LinearOperator):
    ...
    def __init__(self, A, B):
        if A.ndim != 2 or B.ndim != 2:
            raise ValueError('expected arrays representing matrices')
    ...
    def _matvec(self, x):
        return mx.dot(self.A, mx.dot(self.B, x))

    def _rmatvec(self, x):
        return mx.dot(mx.dot(x, self.A), self.B)
...
class TestOnenormest:
    ...
    def test_onenormest_table_3_t_2(self):
        ...
        for i in range(nsamples):
            A = scipy.linalg.inv(mx.random.randn(n, n))
        ...
        observed = mx.array(observed, dtype=float)
        expected = mx.array(expected, dtype=float)
        relative_errors = mx.abs(observed - expected) / expected

        # check the mean underestimation ratio
        underestimation_ratio = observed / expected
        assert_(0.99 < mx.mean(underestimation_ratio) < 1.0)

        # check the max and mean required column resamples
        assert_equal(mx.max(nresample_list), 2)
        assert_(0.05 < mx.mean(nresample_list) < 0.2)

        # check the proportion of norms computed exactly correctly
        nexact = mx.count_nonzero(relative_errors < 1e-14)
        ...
        assert_(3.5 < mx.mean(nmult_list) < 4.5)
    ...
    def test_onenormest_table_4_t_7(self):
        ...
        nresample_list = []
        for i in range(nsamples):
            A = mx.random.randint(-1, 2, size=(n, n))
            est, v, w, nmults, nresamples = _onenormest_core(A, A.T, t, itmax)
            observed.append(est)
            expected.append(scipy.linalg.norm(A, 1))
            nmult_list.append(nmults)
        ...
        # check the mean underestimation ratio
        underestimation_ratio = observed / expected
        assert_(0.90 < mx.mean(underestimation_ratio) < 0.99)

        # check the required column resamples
        assert_equal(mx.max(nresample_list), 0)
    ...
    def test_onenormest_table_5_t_1(self):
        ...
        itmax = 5
        alpha = 1 - 1e-6
        A = -scipy.linalg.inv(mx.identity(n) + alpha*mx.eye(n, k=1))
        first_col = mx.array([1] + [0]*(n-1))
        first_row = mx.array([(-alpha)**i for i in range(n)])
    ...
    def test_onenormest_table_6_t_2(self):
        ...
        nresample_list = []
        for i in range(nsamples):
            A_inv = mx.random.rand(n, n) + 1j * mx.random.rand(n, n)
        ...
        # check the mean underestimation ratio
        underestimation_ratio = observed / expected
        underestimation_ratio_mean = mx.mean(underestimation_ratio)
        assert_(0.90 < underestimation_ratio_mean < 0.99)

        # check the required column resamples
        max_nresamples = mx.max(nresample_list)
    ...
    def test_onenormest_linear_operator(self):
        ...
        n = 6000
        k = 3
        A = mx.random.randn(n, k)
        B = mx.random.randn(k, n)
    ...
    def test_returns(self):
        ...
        assert_allclose(mx.linalg.norm(A.dot(v), 1), s0*mx.linalg.norm(v, 1), rtol=1e-9)
