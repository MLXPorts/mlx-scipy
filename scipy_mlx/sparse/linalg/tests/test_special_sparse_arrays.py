...
from scipy_mlx.sparse.linalg import LaplacianNd
from scipy_mlx.sparse.linalg._special_sparse_arrays import Sakurai
from scipy_mlx.sparse.linalg._special_sparse_arrays import MikotaPair
...
class TestLaplacianNd:
    ...
    def test_eigenvectors(self, grid_shape, bc):
        ...
        atol = n * n * max(mx.finfo(dtype).eps, mx.finfo(mx.double).eps)
        # test the default ``m = None`` every individual eigenvector
        for i in mx.arange(n):
            r = lap.toarray() @ eigenvectors[:, i] - eigenvectors[:, i] * eigenvalues[i]
            assert_allclose(r, mx.zeros_like(r), atol=atol)
        # test every ``m > 0``
        for m in mx.arange(1, n + 1):
            e = lap.eigenvalues(m)
            ev = lap.eigenvectors(m)
            r = lap.toarray() @ ev - ev @ mx.diag(e)
            assert_allclose(r, mx.zeros_like(r), atol=atol)
        # test orthogonality
        assert_allclose(eigenvectors @ eigenvectors.T, mx.eye(n), atol=atol)
        # test full eigendecomposition
        L = eigenvectors @ mx.diag(eigenvalues) @ eigenvectors.T
    ...
    def test_toarray_tosparse_consistency(self, grid_shape, bc):
        lap = LaplacianNd(grid_shape, boundary_conditions=bc)
        n = mx.prod(grid_shape)
        assert_array_equal(lap.toarray(), lap(mx.eye(n)))
...
class TestSakurai:
    ...
    def test_specific_shape(self):
        sak = Sakurai(6)
        assert_array_equal(sak.toarray(), sak(mx.eye(6)))
        a = mx.array(
            [
                [ 5, -4,  1,  0,  0,  0],
                [-4,  6, -4,  1,  0,  0],
        ...
        mx.array_equal(a, sak.toarray())
        mx.array_equal(sak.tosparse().toarray(), sak.toarray())
        ab = mx.array(
            [
                [ 1,  1,  1,  1,  1,  1],
                [-4, -4, -4, -4, -4, -4],
        ...
        mx.array_equal(ab, sak.tobanded())
        e = mx.array(
                [0.03922866, 0.56703972, 2.41789479, 5.97822974,
                 10.54287655, 14.45473055]
            )
        mx.array_equal(e, sak.eigenvalues())
        mx.array_equal(e[:2], sak.eigenvalues(2))

    # `Sakurai` default `dtype` is `mx.int8` as its entries are small integers
    @pytest.mark.parametrize('dtype', ALLDTYPES)
    ...
    def test_dot(self, dtype, argument_dtype):
        """ Test the dot-product for type preservation and consistency.
        """
        result_dtype = mx.promote_types(argument_dtype, dtype)
        n = 5
        sak = Sakurai(n)
        x0 = mx.arange(n)
        x1 = x0.reshape((-1, 1))
        ...
        for x in input_set:
            ...
            assert x.shape == y.shape
            assert mx.can_cast(y.dtype, result_dtype)
            if x.ndim == 2:
                ya = sak.toarray() @ x.astype(argument_dtype)
                mx.array_equal(y, ya)
                assert mx.can_cast(ya.dtype, result_dtype)
                ys = sak.tosparse() @ x.astype(argument_dtype)
                mx.array_equal(y, ys)
                assert mx.can_cast(ys.dtype, result_dtype)
...
class TestMikotaPair:
    ...
    # while `MikotaK` `dtype` can be as small as its default `mx.int32`
    # since its entries are integers, the `MikotaM` involves inverses
    # so its smallest still accurate `dtype` is `mx.float32`
    ...
    def test_specific_shape(self):
        ...
        mik_k = mik.k
        mik_m = mik.m
        assert_array_equal(mik_k.toarray(), mik_k(mx.eye(n)))
        assert_array_equal(mik_m.toarray(), mik_m(mx.eye(n)))
        ...
        mx.array_equal(k, mik_k.toarray())
        mx.array_equal(mik_k.tosparse().toarray(), k)
        kb = mx.array(
            [
                [ 0, -5, -4, -3, -2, -1],
                [11,  9,  7,  5,  3,  1]
        ...
        mx.array_equal(kb, mik_k.tobanded())

        minv = mx.arange(1, n + 1)
        mx.array_equal(mx.diag(1. / minv), mik_m.toarray())
        mx.array_equal(mik_m.tosparse().toarray(), mik_m.toarray())
        mx.array_equal(1. / minv, mik_m.tobanded())

        e = mx.array([ 1,  4,  9, 16, 25, 36])
        mx.array_equal(e, mik.eigenvalues())
        mx.array_equal(e[:2], mik.eigenvalues(2))
    ...
    def test_linearoperator_shape_dtype(self, dtype):
        ...
        assert mikd_k.dtype == mx.float64
        assert mikd_m.shape == (n, n)
        assert mikd_m.dtype == mx.float64
