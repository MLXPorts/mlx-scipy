...
from scipy_mlx._lib._mlx_compat import asarray
from scipy_mlx.sparse import (issparse, SparseEfficiencyWarning,
                          csr_array, csc_array, eye_array, diags_array)
from scipy_mlx.sparse._sputils import (is_pydata_spmatrix, convert_pydata_sparse_to_scipy,
...
def use_solver(**kwargs):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.sparse.linalg import use_solver, spsolve
...
def spilu(A, drop_tol=None, fill_factor=None, drop_rule=None, permc_spec=None,
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.sparse import csc_array
    >>> from scipy_mlx.sparse.linalg import spilu
    >>> A = csc_array([[1., 0., 0.], [5., 0., 2.], [0., -1., 0.]], dtype=float)
    >>> B = spilu(A)
    >>> x = mx.array([1., 2., 3.], dtype=float)
    >>> B.solve(x)
...
def factorized(A):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.sparse.linalg import factorized
    >>> from scipy_mlx.sparse import csc_array
    >>> A = mx.array([[ 3. ,  2. , -1. ],
    ...               [ 2. , -2. ,  4. ],
    ...               [-1. ,  0.5, -1. ]])
...
def spsolve_triangular(A, b, lower=True, overwrite_A=False, overwrite_b=False,
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.sparse import csc_array
    >>> from scipy_mlx.sparse.linalg import spsolve_triangular
...
def is_sptriangular(A):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.sparse import csc_array, eye_array
    >>> from scipy_mlx.sparse.linalg import is_sptriangular
    >>> A = csc_array([[3, 0, 0], [1, -1, 0], [2, 0, 1]], dtype=float)
    >>> is_sptriangular(A)
    (True, False)
...
def spbandwidth(A):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.sparse.linalg import spbandwidth
    >>> from scipy_mlx.sparse import csc_array, eye_array
    >>> A = csc_array([[3, 0, 0], [1, -1, 0], [2, 0, 1]], dtype=float)
    >>> spbandwidth(A)
    (2, 0)
    ...
    if A.format == "dia":
        return max(0, -A.offsets.min()[()]), max(0, A.offsets.max()[()])
    ...
    return max(-mx.min(gap)[()], 0), max(mx.max(gap)[()], 0)
