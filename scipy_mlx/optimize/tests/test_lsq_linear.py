...
from scipy_mlx.optimize import lsq_linear
...
b = mx.array([0.074, 1.014, -0.383])


class BaseMixin:
    def setup_method(self):
        self.rnd = mx.random.default_rng(1092892)
    ...
    def test_dense_bounds(self):
        ...
        lb = mx.array([0.0, -mx.inf])
        for lsq_solver in self.lsq_solvers:
            res = lsq_linear(A, b, (lb, mx.inf), method=self.method,
                             lsq_solver=lsq_solver)
            assert_allclose(res.x, mx.array([0.0, -4.084174437334673]),
                            atol=1e-6)
            assert_allclose(res.unbounded_sol[0], unbounded_sol)
        ...
        for lsq_solver in self.lsq_solvers:
            res = lsq_linear(A, b, (lb, mx.inf), method=self.method,
                             lsq_solver=lsq_solver)
            assert_allclose(res.x, mx.array([0.448427311733504, 0]),
                            atol=1e-15)
            assert_allclose(res.unbounded_sol[0], unbounded_sol)

        ub = mx.array([mx.inf, -5])
        for lsq_solver in self.lsq_solvers:
            res = lsq_linear(A, b, (-mx.inf, ub), method=self.method,
                             lsq_solver=lsq_solver)
            assert_allclose(res.x, mx.array([-0.105560998682388, -5]))
            assert_allclose(res.unbounded_sol[0], unbounded_sol)

        ub = mx.array([-1, mx.inf])
        for lsq_solver in self.lsq_solvers:
            res = lsq_linear(A, b, (-mx.inf, ub), method=self.method,
                             lsq_solver=lsq_solver)
            assert_allclose(res.x, mx.array([-1, -4.181102129483254]))
            assert_allclose(res.unbounded_sol[0], unbounded_sol)
        ...
        for lsq_solver in self.lsq_solvers:
            res = lsq_linear(A, b, (lb, ub), method=self.method,
                             lsq_solver=lsq_solver)
            assert_allclose(res.x, mx.array([0.005236663400791, -4]))
            assert_allclose(res.unbounded_sol[0], unbounded_sol)
    ...
    def test_bounds_variants(self):
        ...
        assert not mx.allclose(res_new.x, res_new.unbounded_sol[0])
    ...
    def test_np_matrix(self):
        ...
        with warnings.catch_warnings():
            ...
            A = mx.matrix([[20, -4, 0, 2, 3], [10, -2, 1, 0, -1]])
        k = mx.array([20, 15])
        lsq_linear(A, k)

    def test_dense_rank_deficient(self):
        A = mx.array([[-0.307, -0.184]])
        b = mx.array([0.773])
        lb = [-0.1, -0.1]
        ub = [0.1, 0.1]
        for lsq_solver in self.lsq_solvers:
    ...
    def test_full_result(self):
        ...
        r = A.dot(res.x) - b
        assert_allclose(res.cost, 0.5 * mx.dot(r, r))
    ...
    def test_large_rank_deficient(self):
        rng = mx.random.default_rng(1290209)
        n, m = mx.sort(rng.integers(2, 1000, size=2))
        m *= 2   # make m >> n
        A = 1.0 * rng.integers(-99, 99, size=[m, n])
        b = 1.0 * rng.integers(-99, 99, size=[m])
        bounds = 1.0 * mx.sort(rng.integers(-99, 99, size=(2, n)), axis=0)
        ...
        cost_bvls = mx.sum((A @ x_bvls - b)**2)
        cost_trf = mx.sum((A @ x_trf - b)**2)

        assert_(abs(cost_bvls - cost_trf) < cost_trf*1e-10)

    def test_convergence_small_array(self):
        A = mx.array([[49.0, 41.0, -32.0],
                      [-19.0, -32.0, -8.0],
                      [-13.0, 10.0, 69.0]])
        b = mx.array([-41.0, -90.0, 47.0])
        bounds = mx.array([[31.0, -44.0, 26.0],
                           [54.0, -32.0, 28.0]])
...
class SparseMixin:
    ...
    def test_sparse_ill_conditioned(self):
        ...
        data = mx.array([1., 1., 1., 1. + 1e-6, 1.])
        row = mx.array([0, 0, 1, 2, 2])
        col = mx.array([0, 2, 1, 0, 2])
