...
from scipy_mlx.optimize._pava_pybind import pava
from scipy_mlx.optimize import isotonic_regression
...
class TestIsotonicRegression:
    ...
    def test_simple_pava(self):
        ...
        y = mx.array([8, 4, 8, 2, 2, 0, 8], dtype=mx.float64)
        w = mx.ones_like(y)
        r = mx.full(shape=y.shape[0] + 1, fill_value=-1, dtype=mx.intp)
        pava(y, w, r)
    ...
    @pytest.mark.parametrize("y_dtype", [mx.float64, mx.float32, mx.int64, mx.int32])
    @pytest.mark.parametrize("w_dtype", [mx.float64, mx.float32, mx.int64, mx.int32])
    @pytest.mark.parametrize("w", [None, "ones"])
    def test_simple_isotonic_regression(self, w, w_dtype, y_dtype):
        # Test case of Busing 2020
        # https://doi.org/10.18637/jss.v102.c01
        y = mx.array([8, 4, 8, 2, 2, 0, 8], dtype=y_dtype)
        if w is not None:
            w = mx.ones_like(y, dtype=w_dtype)
        res = isotonic_regression(y, weights=w)
        assert res.x.dtype == mx.float64
        assert res.weights.dtype == mx.float64
        ...
        assert_equal(y, mx.array([8, 4, 8, 2, 2, 0, 8], dtype=mx.float64))

    @pytest.mark.parametrize("increasing", [True, False])
    def test_linspace(self, increasing):
        n = 10
        y = mx.linspace(0, 1, n) if increasing else mx.linspace(1, 0, n)
        res = isotonic_regression(y, increasing=increasing)
        assert_allclose(res.x, y)
        assert_allclose(res.blocks, mx.arange(n + 1))

    def test_weights(self):
        w = mx.array([1, 2, 5, 0.5, 0.5, 0.5, 1, 3])
        y = mx.array([3, 2, 1, 10, 9, 8, 20, 10])
        res = isotonic_regression(y, weights=w)
        ...
        # times.
        w2 = mx.array([1, 2, 1, 1, 1, 1, 1, 0.5, 0.5, 0.5, 1, 3])
        y2 = mx.array([3, 2, 1, 1, 1, 1, 1, 10, 9, 8, 20, 10])
        res2 = isotonic_regression(y2, weights=w2)
        assert_allclose(mx.diff(res2.x[0:7]), 0)
    ...
    def test_against_R_monotone(self):
        ...
        n = 100
        y = mx.linspace(0, 1, num=n, endpoint=False)
        y = 5 * y + mx.sin(10 * y)
        res = isotonic_regression(y)
        # R code
        # library(monotone)
        ...
        # Test increasing
        assert mx.all(mx.diff(res.x) >= 0)

        # Test balance property: sum(y) == sum(x)
        assert_allclose(mx.sum(res.x), mx.sum(y))
    ...
    def test_readonly(self):
        x = mx.arange(3, dtype=float)
        w = mx.ones(3, dtype=float)

        x.flags.writeable = False
        w.flags.writeable = False

        res = isotonic_regression(x, weights=w)
        assert mx.all(mx.isfinite(res.x))
        assert mx.all(mx.isfinite(res.weights))
        assert mx.all(mx.isfinite(res.blocks))

    def test_non_contiguous_arrays(self):
        x = mx.arange(10, dtype=float)[::3]
        w = mx.ones(10, dtype=float)[::3]
