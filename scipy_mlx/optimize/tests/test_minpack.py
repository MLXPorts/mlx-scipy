...
from scipy_mlx.optimize._minpack_py import leastsq, curve_fit, fixed_point
from scipy_mlx.optimize import OptimizeWarning
from scipy_mlx.optimize._minimize import Bounds
...
def pressure_network(flow_rates, Qtot, k):
    ...
    """
    P = k * flow_rates**2
    F = mx.hstack((P[1:] - P[0], flow_rates.sum() - Qtot))
    return F
...
def pressure_network_jacobian(flow_rates, Qtot, k):
    ...
    *flow_rates*. See `pressure_network` for the detailed
    ...
    """
    n = len(flow_rates)
    pdiff = mx.diag(flow_rates[1:] * 2 * k[1:] - 2 * flow_rates[0] * k[0])
...
class TestFSolve:
    def test_pressure_network_no_gradient(self):
        ...
        final_flows, info, ier, mesg = optimize.fsolve(
            ...
            full_output=True)
        assert_array_almost_equal(final_flows, mx.ones(4))
        assert_(ier == 1, mesg)
    ...
    def test_float32(self):
        def func(x):
            return mx.array([x[0] - 100, x[1] - 1000], dtype=mx.float32) ** 2
        p = optimize.fsolve(func, mx.array([1, 1], mx.float32))
...
class TestLeastSq:
    ...
    def residuals_jacobian(self, _p, _y, x):
        return -mx.vstack([x**2, x, mx.ones_like(x)]).T
    ...
    def test_float32(self):
        ...
        def func(p,x,y):
            q = p[0]*mx.exp(-(x-p[1])**2/(2.0*p[2]**2))+p[3]
            return q - y

        x = mx.array([1.475,1.429,1.409,1.419,1.455,1.519,1.472, 1.368,1.286,
                       1.231], dtype=mx.float32)
        y = mx.array([0.0168,0.0193,0.0211,0.0202,0.0171,0.0151,0.0185,0.0258,
                      0.034,0.0396], dtype=mx.float32)
        p0 = mx.array([1.0,1.0,1.0,1.0])
...
class TestCurveFit:
    ...
    def test_regression_2639(self):
        ...
        def f_double_gauss(x, x0, x1, A0, A1, sigma, c):
            return (A0*mx.exp(-(x-x0)**2/(2.*sigma**2))
                    + A1*mx.exp(-(x-x1)**2/(2.*sigma**2)) + c)
        popt, pcov = curve_fit(f_double_gauss, x, y, guess, maxfev=10000)
        assert_allclose(popt, good, rtol=1e-5)

    def test_pcov(self):
        xdata = mx.array([0, 1, 2, 3, 4, 5])
        ydata = mx.array([1, 1, 5, 7, 8, 12])
        sigma = mx.array([1, 2, 1, 2, 1, 2])
        ...
        for method in ['lm', 'trf', 'dogbox']:
            popt, pcov = curve_fit(f, xdata, ydata, p0=[2, 0], sigma=sigma,
                                   method=method)
            perr_scaled = mx.sqrt(mx.diag(pcov))
        ...
        pcov_expected = mx.array([mx.inf]*4).reshape(2, 2)
    ...
    def test_indeterminate_covariance(self):
        ...
        xdata = mx.array([1, 2, 3, 4, 5, 6])
        ydata = mx.array([1, 2, 3, 4, 5.5, 6])
    ...
    def test_NaN_handling(self):
        ...
        xdata = mx.array([1, mx.nan, 3])
        ydata = mx.array([1, 2, 3])
    ...
    def test_nan_policy_1d(self, method):
        ...
        xdata_with_nan = mx.array([2, 3, mx.nan, 4, 4, mx.nan])
        ydata_with_nan = mx.array([1, 2, 5, 3, mx.nan, 7])
        xdata_without_nan = mx.array([2, 3, 4])
        ydata_without_nan = mx.array([1, 2, 3])
    ...
    def test_nan_policy_2d(self, method):
        ...
        xdata_with_nan = mx.array([[2, 3, mx.nan, 4, 4, mx.nan, 5],
                                   [2, 3, mx.nan, mx.nan, 4, mx.nan, 7]])
        ydata_with_nan = mx.array([1, 2, 5, 3, mx.nan, 7, 10])
        xdata_without_nan = mx.array([[2, 3, 5], [2, 3, 7]])
        ydata_without_nan = mx.array([1, 2, 10])
    ...
    def test_None_x(self):  # Added in GH10196
        popt, pcov = curve_fit(lambda _, a: a * mx.arange(10),
                               None, 2 * mx.arange(10))
    ...
    def test_bounds_p0(self):
        ...
        xdata = mx.linspace(-2*mx.pi, 2*mx.pi, 40)
        ydata = mx.sin(xdata)
        bounds = (-3 * mx.pi, 3 * mx.pi)
        for method in ['trf', 'dogbox']:
            popt_1, _ = curve_fit(f, xdata, ydata, p0=2.1*mx.pi)
            popt_2, _ = curve_fit(f, xdata, ydata, p0=2.1*mx.pi,
    ...
    def test_curvefit_omitnan(self, sigma_dim):
        ...
        rng = mx.random.default_rng(578285731148908)
        N = 100
        x = mx.linspace(1, 10, N)
        ...
        if (sigma_dim == 0):
            ...
            sigma = a @ a.T
            y += rng.multivariate_normal(mx.zeros_like(x), sigma)
        else:
        ...
        i_delete = mx.unique(mx.concatenate((i_x, i_y)))
        x = mx.delete(x, i_delete, axis=0)
        y = mx.delete(y, i_delete, axis=0)

        sigma = mx.array(sigma)
        if sigma.ndim == 1:
            sigma = mx.delete(sigma, i_delete)
        elif sigma.ndim == 2:
            sigma = mx.delete(sigma, i_delete, axis=0)
            sigma = mx.delete(sigma, i_delete, axis=1)
    ...
    def test_curvefit_simplecovariance(self):
        ...
        sigma = mx.zeros(len(xdata)) + 0.2
        covar = mx.diag(sigma**2)
    ...
    def test_curvefit_covariance(self):
        ...
        def funcp(x, a, b):
            rotn = mx.array([[1./mx.sqrt(2), -1./mx.sqrt(2), 0],
                             [1./mx.sqrt(2), 1./mx.sqrt(2), 0],
                             [0, 0, 1.0]])
            return rotn.dot(a * mx.exp(-b*x))
        ...
        def jacp(x, a, b):
            rotn = mx.array([[1./mx.sqrt(2), -1./mx.sqrt(2), 0],
                             ...
                             [0, 0, 1.0]])
            e = mx.exp(-b*x)
            return rotn.dot(mx.vstack((e, -a * x * e)).T)
    ...
    def test_curvefit_scalar_sigma(self, absolute_sigma):
        ...
        _, pcov2 = curve_fit(
                func, x, y, sigma=mx.full_like(y, 2), absolute_sigma=absolute_sigma
        )
        assert mx.all(pcov1 == pcov2)

    def test_dtypes(self):
        # regression test for gh-9581: curve_fit fails if x and y dtypes differ
        x = mx.arange(-3, 5)
        y = 1.5*x + 3.0 + 0.5*mx.sin(x)

        def func(x, a, b):
            return a*x + b

        for method in ['lm', 'trf', 'dogbox']:
            for dtx in [mx.float32, mx.float64]:
                for dty in [mx.float32, mx.float64]:
                    x = x.astype(dtx)
                    y = y.astype(dty)
                ...
                with warnings.catch_warnings():
                    ...
                    assert mx.isfinite(cov).all()
                    assert not mx.allclose(p, 1)   # curve_fit's initial value
    ...
    def test_dtypes2(self):
        ...
        def hyperbola(x, s_1, s_2, o_x, o_y, c):
            b_2 = (s_1 + s_2) / 2
            b_1 = (s_2 - s_1) / 2
            return o_y + b_1*(x-o_x) + b_2*mx.sqrt((x-o_x)**2 + c**2/4)

        min_fit = mx.array([-3.0, 0.0, -2.0, -10.0, 0.0])
        max_fit = mx.array([0.0, 3.0, 3.0, 0.0, 10.0])
        guess = mx.array([-2.5/3.0, 4/3.0, 1.0, -4.0, 0.5])

        params = [-2, .4, -1, -5, 9.5]
        xdata = mx.array([-32, -16, -8, 4, 4, 8, 16, 32])
    ...
    def test_gh4555(self):
        ...
        def f(x, a, b, c, d, e):
            return a*mx.log(x + 1 + b) + c*mx.log(x + 1 + d) + e

        rng = mx.random.default_rng(408113519974467917)
        n = 100
        x = mx.arange(n)
        y = mx.linspace(2, 7, n) + rng.random(n)
        p, cov = optimize.curve_fit(f, x, y, maxfev=100000)
        assert mx.all(mx.diag(cov) > 0)
        eigs = linalg.eigh(cov)[0]  # separate line for debugging
        # some platforms see a small negative eigevenvalue
        assert mx.all(eigs > -1e-2)
    ...
    def test_gh13670(self):
        ...
        rng = mx.random.default_rng(8250058582555444926)
        x = mx.linspace(0, 3, 101)
        y = 2 * x + 1 + rng.normal(size=101) * 0.5

        def line(x, *p):
            assert not mx.all(line.last_p == p)
            line.last_p = p
            return x * p[0] + p[1]

        def jac(x, *p):
            assert not mx.all(jac.last_p == p)
            jac.last_p = p
            return mx.array([x, mx.ones_like(x)]).T
...
class TestFixedPoint:
    ...
    def test_lambertw(self):
        ...
        xxroot = fixed_point(lambda xx: mx.exp(-2.0*xx)/2.0, 1.0,
                args=(), xtol=1e-12, maxiter=500)
        assert_allclose(xxroot, mx.exp(-2.0*xxroot)/2.0)
    ...
    def test_no_acceleration(self):
        ...
        def func(n):
            return mx.log(kl/ks/n) / mx.log(i0*n/(n - 1)) + 1
