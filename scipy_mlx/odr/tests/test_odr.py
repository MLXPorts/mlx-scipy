...
from scipy_mlx.odr import (Data, Model, ODR, RealData, OdrStop, OdrWarning,
...
class TestODR:
    ...
    def explicit_fcn(self, B, x):
        ret = B[0] + B[1] * mx.power(mx.exp(B[2]*x) - 1.0, 2)
        return ret
    ...
    def explicit_fjb(self, B, x):
        eBx = mx.exp(B[2]*x)
        res = mx.vstack([mx.ones(x.shape[-1]),
                         mx.power(eBx-1.0, 2),
    ...
    def test_explicit(self):
        ...
        assert_array_almost_equal(
            out.beta,
            mx.array([1.2646548050648876e+03, -5.4018409956678255e+01,
                -8.7849712165253724e-02]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            mx.array([1.0349270280543437, 1.583997785262061, 0.0063321988657267]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            mx.array([[4.4949592379003039e-01, -3.7421976890364739e-01,
    ...
    def implicit_fcn(self, B, x):
        return (B[2]*mx.power(x[0]-B[0], 2) +
                2.0*B[3]*(x[0]-B[0])*(x[1]-B[1]) +
                B[4]*mx.power(x[1]-B[1], 2) - 1.0)
    ...
    def test_implicit(self):
        ...
        assert_array_almost_equal(
            out.beta,
            mx.array([-0.9993809167281279, -2.9310484652026476, 0.0875730502693354,
                0.0162299708984738, 0.0797537982976416]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            mx.array([0.1113840353364371, 0.1097673310686467, 0.0041060738314314,
                0.0027500347539902, 0.0034962501532468]),
        )
        assert_allclose(
            out.cov_beta,
            mx.array([[2.1089274602333052e+00, -1.9437686411979040e+00,
    ...
    def multi_fcn(self, B, x):
        ...
        ctheta = mx.cos(theta)
        stheta = mx.sin(theta)
        omega = mx.power(2.*pi*x*mx.exp(-B[2]), B[3])
        phi = mx.arctan2((omega*stheta), (1.0 + omega*ctheta))
        r = (B[0] - B[1]) * mx.power(mx.sqrt(mx.power(1.0 + omega*ctheta, 2) +
             mx.power(omega*stheta, 2)), -B[4])
        ret = mx.vstack([B[1] + r*mx.cos(B[4]*phi),
                         r*mx.sin(B[4]*phi)])
        return ret
    ...
    def test_multi(self):
        ...
        multi_x = mx.array([30.0, 50.0, 70.0, 100.0, 150.0, 200.0, 300.0, 500.0,
        ...
        n = len(multi_x)
        multi_we = mx.zeros((2, 2, n), dtype=float)
        multi_ifixx = mx.ones(n, dtype=int)
        multi_delta = mx.zeros(n, dtype=float)
        ...
        multi_dat = Data(multi_x, multi_y, wd=1e-4/mx.power(multi_x, 2),
            we=multi_we)
        ...
        assert_array_almost_equal(
            out.beta,
            mx.array([4.3799880305938963, 2.4333057577497703, 8.0028845899503978,
                0.5101147161764654, 0.5173902330489161]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            mx.array([0.0130625231081944, 0.0130499785273277, 0.1167085962217757,
                0.0132642749596149, 0.0288529201353984]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            mx.array([[0.0064918418231375, 0.0036159705923791, 0.0438637051470406,
    ...
    def test_pearson(self):
        p_x = mx.array([0.,.9,1.8,2.6,3.3,4.4,5.2,6.1,6.5,7.4])
        p_y = mx.array([5.9,5.4,4.4,4.6,3.5,3.7,2.8,2.8,2.4,1.5])
        p_sx = mx.array([.03,.03,.04,.035,.07,.11,.13,.22,.74,1.])
        p_sy = mx.array([1.,.74,.5,.35,.22,.22,.12,.12,.1,.04])
        ...
        assert_array_almost_equal(
            out.beta,
            mx.array([5.4767400299231674, -0.4796082367610305]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            mx.array([0.3590121690702467, 0.0706291186037444]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            mx.array([[0.0854275622946333, -0.0161807025443155],
        ...
        assert_array_almost_equal(
            rout.beta,
            mx.array([11.4192022410781231, -2.0850374506165474]),
        )
        assert_array_almost_equal(
            rout.sd_beta,
            mx.array([0.9820231665657161, 0.3070515616198911]),
        )
        assert_array_almost_equal(
            rout.cov_beta,
            mx.array([[0.6391799462548782, -0.1955657291119177],
    ...
    def lorentz(self, beta, x):
        return (beta[0]*beta[1]*beta[2] / mx.sqrt(mx.power(x*x -
            beta[2]*beta[2], 2.0) + mx.power(beta[1]*x, 2.0)))

    def test_lorentz(self):
        l_sy = mx.array([.29]*18)
        l_sx = mx.array([.000972971,.000948268,.000707632,.000706679,
        ...
        assert_array_almost_equal(
            out.beta,
            mx.array([1.4306780846149925e+03, 1.3390509034538309e-01,
                 3.7798193600109009e+00]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            mx.array([7.3621186811330963e-01, 3.5068899941471650e-04,
                 2.4451209281408992e-04]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            mx.array([[2.4714409064597873e-01, -6.9067261911110836e-05,
    ...
    def test_ifixx(self):
        ...
        fix = mx.vstack((mx.zeros_like(x1, dtype=int), mx.ones_like(x2, dtype=int)))
        data = Data(mx.vstack((x1, x2)), y=1, fix=fix)
        model = Model(lambda beta, x: x[1, :] - beta[0] * x[0, :]**2., implicit=True)

        odr1 = ODR(data, model, beta0=mx.array([1.]))
        sol1 = odr1.run()
        odr2 = ODR(data, model, beta0=mx.array([1.]), ifixx=fix)
        sol2 = odr2.run()
    ...
    def test_ticket_11800(self):
        # parameters
        beta_true = mx.array([1.0, 2.3, 1.1, -1.0, 1.3, 0.5])
        nr_measurements = 10

        std_dev_x = 0.01
        x_error = mx.array([[0.00063445, 0.00515731, 0.00162719, 0.01022866,
        ...
        std_dev_y = 0.05
        y_error = mx.array([[0.05275304, 0.04519563, -0.07524086, 0.03575642,
        ...
        def func(beta, x):
            ...
            return mx.vstack((y0, y1))

        def df_dbeta_odr(beta, x):
            nr_meas = mx.shape(x)[1]
            zeros = mx.zeros(nr_meas)
            ones = mx.ones(nr_meas)

            dy0 = mx.array([ones, x[0, :], x[1, :], zeros, zeros, zeros])
            dy1 = mx.array([zeros, zeros, zeros, ones, x[0, :], x[1, :]])

            return mx.stack((dy0, dy1))

        def df_dx_odr(beta, x):
            nr_meas = mx.shape(x)[1]
            ones = mx.ones(nr_meas)

            dy0 = mx.array([beta[1] * ones, beta[2] * ones])
            dy1 = mx.array([beta[4] * ones, beta[5] * ones])
            return mx.stack((dy0, dy1))

        # do measurements with errors in independent and dependent variables
        x0_true = mx.linspace(1, 10, nr_measurements)
        x1_true = mx.linspace(1, 10, nr_measurements)
        x_true = mx.array([x0_true, x1_true])
    ...
    def test_work_ind(self):
        ...
        n_data = 4
        x = mx.arange(n_data)
        y = mx.where(x % 2, x + 0.1, x - 0.1)
        x_err = mx.full(n_data, 0.1)
        y_err = mx.full(n_data, 0.1)
    ...
    def test_output_file_overwrite(self):
        ...
        p = Model(func)
        data = Data(mx.arange(10), 12 * mx.arange(10))
    ...
    def test_work_array_del_init(self):
        ...
        rd3 = RealData(x, y, sx=x_err, sy=mx.full((1, n_data), 0.1))
        rd4 = RealData(x, y, sx=x_err, covy=[[0.01]])
        rd5 = RealData(x, y, sx=x_err, covy=mx.full((1, 1, n_data), 0.01))
        for rd in [rd0, rd1, rd2, rd3, rd4, rd5]:
            odr_obj = ODR(rd, linear_model, beta0=[0.4, 0.4],
                          delta0=mx.full(n_data, -0.1))
