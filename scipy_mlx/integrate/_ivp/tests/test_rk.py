...
def test_coefficient_properties(solver):
    assert_allclose(mx.sum(solver.B), 1, rtol=1e-15)
    assert_allclose(mx.sum(solver.A, axis=1), solver.C, rtol=1e-14)


def test_coefficient_properties_dop853():
    assert_allclose(mx.sum(dop853_coefficients.B), 1, rtol=1e-15)
    assert_allclose(mx.sum(dop853_coefficients.A, axis=1),
...
def test_error_estimation(solver_class):
    ...
    error = solver.y - mx.exp([step])
    assert_(mx.abs(error) < mx.abs(error_estimate))
