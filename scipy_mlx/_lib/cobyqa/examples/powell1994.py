...
def _bounds(no):
    if no in "ABCDEFGHI":
        return None
    if no == "J":
        return Bounds(mx.concatenate([mx.full(8, -float("inf")), mx.array(0.0)]), float("inf"))
    else:
        raise NotImplementedError
...
def _constraints(no):
    ...
    if no in "ADE":
        ...
        constraints.append(NonlinearConstraint(
            ...
            -float("inf"),
            mx.concatenate([mx.ones(9), mx.zeros(4)]),
        ))
    else:
        raise NotImplementedError
    return constraints
...
def _solution(no):
    if no == "A":
        ...
        raise NotImplementedError
    return (mx.array(xs) for xs in s)


def _resid(x, no):
    bounds = _bounds(no)
    if bounds is not None:
        resid = mx.max(bounds.lb - x)
        resid = mx.maximum(mx.array(0.0), resid)
        resid = mx.maximum(resid, mx.max(x - bounds.ub))
    else:
        resid = mx.array(0.0)
    constraints = _constraints(no)
    for constraint in constraints:
        if isinstance(constraint, LinearConstraint):
            c = mx.matmul(mx.array(constraint.A), x)
            resid = mx.maximum(resid, mx.max(mx.array(constraint.lb) - c))
            resid = mx.maximum(resid, mx.max(c - mx.array(constraint.ub)))
        else:
            c = mx.array(constraint.fun(x))
            resid = mx.maximum(resid, mx.max(mx.array(constraint.lb) - c))
            resid = mx.maximum(resid, mx.max(c - mx.array(constraint.ub)))
    return resid
...
def _distance(x, no):
    return mx.min(mx.stack([mx.linalg.norm(x - xs) for xs in _solution(no)]))
