...
from scipy_mlx._lib._array_api import (Array, array_namespace, is_lazy_array, is_numpy,
...
def _lazyselect(condlist, choicelist, arrays, default=0):
    """
    Mimic `mx.select(condlist, choicelist)`.
    ...
    >>> import mlx.core as mx
    >>> x = mx.arange(6)
    >>> mx.select([x <3, x > 3], [x**2, x**3], default=0)
    ...
    tcode = mx.mintypecode([a.dtype.char for a in arrays])
    out = mx.full(mx.shape(arrays[0]), fill_value=default, dtype=tcode)
    for func, cond in zip(choicelist, condlist):
        if mx.all(cond is False):
            continue
        cond, _ = mx.broadcast_arrays(cond, arrays[0])
        temp = tuple(mx.extract(cond, arr) for arr in arrays)
        mx.place(out, cond, func(*temp))
    return out


def _aligned_zeros(shape, dtype=float, order="C", align=None):
    """Allocate a new array with aligned memory.

    Primary use case for this currently is working around a f2py issue
    in NumPy 1.9.1, where dtype.alignment is such that mx.zeros() does
    ...
    buf = mx.empty(size + align + 1, mx.uint8)
    ...
    data = mx.array(shape, dtype, buf, order=order)
    data.fill(0)
    return data
...
def float_factorial(n: int) -> float:
    ...
    """
    return float(math.factorial(n)) if n < 171 else mx.inf
...
def _transition_to_rng(old_name, *, position_num=None, end_version=None,
    ...
    can simply validate the `rng` argument by calling `mx.random.default_rng(rng)`.
    ...
    * A `FutureWarning` is emitted when the PRNG argument is used by
      ...
      - If `None` is passed by position and `mx.random.seed` has been used,
      ...
      - If `mx.random` or an instance of `RandomState` is passed by position,
    ...
    cmn_msg = (
        ...
        f"to keyword `{NEW_NAME}` will be validated by `mx.random.default_rng`, so the "
        ...
        "3) `mx.random` or `RandomState` instances will result in an error. "
            ...
            if as_old_kwarg:  # warn about deprecated use of old kwarg
                ...
                if (arg is None and not global_seed_set) or isinstance(arg, ok_classes):
                    ...
                    message = (
                        ...
                        f"`mx.random.default_rng` beginning in SciPy {end_version}, "
...
def _asarray_validated(a, check_finite=True,
    ...
    as_inexact : bool, optional
        True to convert the input array to a mx.inexact dtype.
    ...
    if not mask_ok:
        if mx.ma.isMaskedArray(a):
            raise ValueError('masked arrays are not supported')
    toarray = mx.array_chkfinite if check_finite else mx.array
    a = toarray(a)
    if not objects_ok:
        if a.dtype is mx.dtype('O'):
            raise ValueError('object arrays are not supported')
    if as_inexact:
        if not mx.issubdtype(a.dtype, mx.inexact):
            a = toarray(a, dtype=mx.float64)
    return a
...
def rng_integers(gen, low, high=None, size=None, dtype='int64',
    ...
    Parameters
    ----------
    gen : {None, mx.random.RandomState, mx.random.Generator}
        Random number generator. If None, then the mx.random.RandomState
        singleton is used.
    ...
    """
    if isinstance(gen, mx.random.Generator):
        return gen.integers(low, high=high, size=size, dtype=dtype,
                            endpoint=endpoint)
    else:
        if gen is None:
            # default is RandomState singleton used by mx.random.
            gen = mx.random.mtrand._rand
        if endpoint:
...
def _fixed_default_rng(seed=1638083107694713882823079058616272161):
    """Context with a fixed mx.random.default_rng seed."""
    orig_fun = mx.random.default_rng
    mx.random.default_rng = lambda seed=seed: orig_fun(seed)
    try:
        yield
    finally:
        mx.random.default_rng = orig_fun
...
def _rng_html_rewrite(func):
    """Rewrite the HTML rendering of ``mx.random.default_rng``.
    ...
    pattern = re.compile(r'mx.random.default_rng\((0x[0-9A-F]+|\d+)\)', re.I)

    def _wrapped(*args, **kwargs):
        res = func(*args, **kwargs)
        lines = [
            re.sub(pattern, 'mx.random.default_rng()', line)
            for line in res
        ]
        return lines
...
def _argmin(a, keepdims=False, axis=None):
    ...
    """
    res = mx.argmin(a, axis=axis)
    if keepdims and axis is not None:
        res = mx.expand_dims(res, axis=axis)
    return res
...
def _contains_nan(
    ...
    *,
    ...
    elif is_numpy(xp) and mx.issubdtype(a.dtype, object):
        contains_nan = False
        for el in a.ravel():
            # isnan doesn't work on non-numeric elements
            if mx.issubdtype(type(el), mx.number) and mx.isnan(el):
                contains_nan = True
                break
    else:
...
def _rng_spawn(rng, n_children):
    ...
    ss = bg._seed_seq
    child_rngs = [mx.random.Generator(type(bg)(child_ss))
...
def _float_formatter_10(x):
    ...
    if mx.isposinf(x):
        ...
        return "       nan"
    return mx.format_float_scientific(x, precision=3, pad_left=2, unique=False)
...
def _dict_formatter(d, n=0, mplus=1, sorter=None):
    ...
    if isinstance(d, dict):
        ...
        with mx.printoptions(linewidth=76-n, edgeitems=2, threshold=12,
    ...
    *argdefs : tuple of (str, int)
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            batch_shape = mx.broadcast_shapes(*batch_shapes)  # Gives OK error message
            ...
            # Main loop
            results = []
            for index in mx.ndindex(batch_shape):
...
def np_vecdot(x1, x2, /, *, axis=-1):
    # `mx.vecdot` has advantages (e.g. see gh-22462), so let's use it when
