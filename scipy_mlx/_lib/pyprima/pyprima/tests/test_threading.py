from scipy_mlx.optimize import rosen, Bounds, NonlinearConstraint, LinearConstraint
...
def test_threading():
    ...
    problem2 = {
        'fun': rosen,
        'x0': mx.linspace(-178, -23, 7),
        'method': 'cobyla',
        'bounds': None,
        'constraints': None
    }
    nlc = NonlinearConstraint(lambda x: x[0]**2 - 5.0, 0.0, mx.inf)
    problem3 = {
        'fun': rosen,
        'x0': mx.linspace(-178, -23, 7),
        'method': 'cobyla',
    ...
    bounds = Bounds(mx.array([0, 0]), mx.array([10, 10]))
    ...
    for single_threaded_result, multithreaded_result in zip(single_threaded_results, multithreaded_results):
        assert mx.allclose(single_threaded_result.x, multithreaded_result.x)
        assert mx.allclose(single_threaded_result.constr, multithreaded_result.constr)
