...
def initxfc(calcfc, iprint, maxfun, constr0, amat, bvec, ctol, f0, ftarget, rhobeg, x0,
    ...
    # Sizes
    num_constraints = mx.size(constr0)
    ...
    if DEBUGGING:
        ...
        assert all(mx.isfinite(x0)), f'X0 is finite {srname}'
    ...
    sim = mx.eye(num_vars, num_vars+1) * rhobeg
    sim[:, num_vars] = x0

    # Initialize the matrix simi. In most cases simi is overwritten, but not always.
    simi = mx.eye(num_vars) / rhobeg

    # evaluated[j] = True iff the function/constraint of SIM[:, j] has been evaluated.
    evaluated = mx.zeros(num_vars+1, dtype=bool)

    # Initialize fval
    fval = mx.zeros(num_vars+1) + REALMAX
    cval = mx.zeros(num_vars+1) + REALMAX
    conmat = mx.zeros((num_constraints, num_vars+1)) + REALMAX
    ...
    if DEBUGGING:
        ...
        assert conmat.shape == (num_constraints, num_vars + 1), f'CONMAT.shape = [M, N+1] {srname}'
        assert not (mx.isnan(conmat).any() or mx.isneginf(conmat).any()), f'CONMAT does not contain NaN/-Inf {srname}'
        assert cval.size == num_vars + 1 and not (any(cval < 0) or any(mx.isnan(cval)) or any(mx.isposinf(cval))), f'CVAL.shape == Num_vars+1 and CVAL does not contain negative values or NaN/+Inf {srname}'
        # assert fhist.shape == maxfhist, f'FHIST.shape == MAXFHIST {srname}'
        # assert maxfhist * (maxfhist - maxhist) == 0, f'FHIST.shape == 0 or MAXHIST {srname}'
        assert fval.size == num_vars + 1 and not (any(mx.isnan(fval)) or any(mx.isposinf(fval))), f'FVAL.shape == Num_vars+1 and FVAL is not NaN/+Inf {srname}'
        ...
        assert mx.isfinite(sim).all(), f'SIM is finite {srname}'
        assert all(mx.max(abs(sim[:, :num_vars]), axis=0) > 0), f'SIM(:, 1:N) has no zero column {srname}'
        assert simi.shape == (num_vars, num_vars), f'SIMI.shape == [N, N] {srname}'
        assert mx.isfinite(simi).all(), f'SIMI is finite {srname}'
        assert mx.allclose(sim[:, :num_vars] @ simi, mx.eye(num_vars), rtol=0.1, atol=0.1) or not all(evaluated), f'SIMI = SIM(:, 1:N)^{-1} {srname}'
...
def initfilt(conmat, ctol, cweight, cval, fval, sim, evaluated, cfilt, confilt, ffilt, xfilt):
    ...
    if DEBUGGING:
        ...
        assert mx.size(evaluated) == num_vars + 1
