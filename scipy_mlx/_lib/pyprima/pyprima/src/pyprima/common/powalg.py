...
def qradd_Rdiag(c, Q, Rdiag, n):
    ...
    Case 1. If C is not in range(A) (theoretically, it implies N < M), then the new matrix is mx.hstack([A, C])
    Case 2. If C is in range(A), then the new matrix is mx.hstack([A[:, :n-1], C])
    N.B.:
    0. Instead of R, this subroutine updates Rdiag, which is mx.diag(R), with a size at most M and at
    ...
    cq = mx.array([0 if isminor(cqi, cqai) else cqi for cqi, cqai in zip(cq, cqa)])
    ...
    # Update Rdiag so that Rdiag[n] = cq[n] = mx.dot(c, q[:, n]). Note that N may be been augmented.
...
def qrexc_Rdiag(A, Q, Rdiag, i):  # Used in COBYLA
    ...
    N.B.:
    0. Instead of R, this function updates Rdiag, which is mx.diag(R), the size being n.
    ...
    for k in range(i, n-1):
        ...
        # hypt = mx.sqrt(Rdiag[k+1]**2 + mx.dot(Q[:, k], A[:, k+1])**2)
