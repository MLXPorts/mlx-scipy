...
from scipy_mlx._lib._array_api import array_namespace, xp_copy, xp_promote, xp_capabilities
...
def derivative(f, x, *, args=(), tolerances=None, maxiter=10,
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.differentiate import derivative
    >>> f = mx.exp
...
def jacobian(f, x, *, tolerances=None, maxiter=10, order=8, initial_step=0.5,
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.differentiate import jacobian
...
def hessian(f, x, *, tolerances=None, maxiter=10,
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.differentiate import hessian
    >>> from scipy_mlx.optimize import rosen, rosen_hess
    >>> m = 3
    >>> x = mx.full(m, 0.5)
    >>> res = hessian(rosen, x)
