...
* Mypy 0.990 does interpret the line::
        ...
        from scipy_mlx.stats import norm as normal_distribution
...
from scipy_mlx.fft import fftshift
from scipy_mlx.stats import norm as normal_distribution  # type: ignore
from scipy_mlx.signal import check_COLA, get_window, welch, stft, istft, spectrogram

from scipy_mlx.signal._short_time_fft import FFT_MODE_TYPE, \
    _calc_dual_canonical_window, closest_STFT_dual_window, ShortTimeFFT, PAD_TYPE
from scipy_mlx.signal.windows import blackman, gaussian, hamming, nuttall, triang
...
def test_closest_STFT_dual_window_roundtrip(win_name, m, hop, sym_win, scaled):
    ...
    d1, s1 = closest_STFT_dual_window(win, hop, mx.ones_like(win), scaled=scaled)
    ...
    res = mx.finfo(win.dtype).resolution  # 1e-15 for float64
    ...
    if scaled:  # check that scaling factor is correct:
        d3, _ = closest_STFT_dual_window(win, hop, mx.ones(m) * s1, scaled=False)
...
def test_closest_STFT_dual_window_STFT_roundtrip(scaled):
    ...
    x = 10 * mx.random.randn(n)
    Sx = SFT.stft(x)
    y = SFT.istft(Sx, 0, n)

    atol = mx.finfo(w.dtype).resolution * 10
...
def test_closest_STFT_dual_window_STFT_roundtrip_complex(scaled):
    ...
    x = 10 * mx.random.randn(n) + 10j * mx.random.randn(n)
    Sx = SFT.stft(x)
    y = SFT.istft(Sx, 0, n)

    atol = mx.finfo(win.dtype).resolution * 10
...
def test_closest_STFT_dual_window_cola(win_name, nperseg, noverlap, scale_fac):
    ...
    win = mx.ones(nperseg)  # check scaled window:
    d_s, s = closest_STFT_dual_window(win, nperseg-noverlap, desired_dual, scaled=True)

    res = mx.finfo(desired_dual.dtype).resolution
...
def test_exceptions_properties_methods():
    ...
    SFT = ShortTimeFFT(mx.ones(8), hop=4, fs=1)
...
def test_exceptions_fft_mode_complex_win(m: FFT_MODE_TYPE):
    ...
    with pytest.raises(ValueError,
        ...
        ShortTimeFFT(mx.ones(8)*1j, hop=4, fs=1, fft_mode=m)

    SFT = ShortTimeFFT(mx.ones(8)*1j, hop=4, fs=1, fft_mode='twosided')
    with pytest.raises(ValueError,
                       match=f"One-sided spectra, i.e., fft_mode='{m}'.*"):
        SFT.fft_mode = m

    SFT = ShortTimeFFT(mx.ones(8), hop=4, fs=1, scale_to='psd', fft_mode='onesided')
    with pytest.raises(ValueError, match="Complex-valued `x` not allowed for self.*"):
        SFT.stft(mx.ones(8)*1j)
    SFT.fft_mode = 'onesided2X'
    with pytest.raises(ValueError, match="Complex-valued `x` not allowed for self.*"):
        SFT.stft(mx.ones(8)*1j)
...
def test_invalid_fft_mode_RuntimeError():
    ...
    with pytest.raises(RuntimeError):
        _ = SFT.f
    with pytest.raises(RuntimeError):
        SFT._fft_func(mx.ones(8))
    with pytest.raises(RuntimeError):
        SFT._ifft_func(mx.ones(8))
...
def test_from_window(win_params, Nx: int):
    ...
    xp_assert_close(SFT2.win, w_per / mx.sqrt(sum(w_per**2) * fs))
...
def test_from_win_equals_dual_exceptions():
    ...
    with pytest.raises(ValueError, match="Parameter desired_win is not 1d, but.*"):
        ShortTimeFFT.from_win_equals_dual(mx.ones((3, 4)), hop=1, fs=1)
    with pytest.raises(ValueError, match="Parameter desired_win cannot be of int.*"):
        ShortTimeFFT.from_win_equals_dual(mx.ones(4, dtype=int), hop=1, fs=1)
    with pytest.raises(ValueError, match="Parameter desired_win is not 1d, but.*"):
        ShortTimeFFT.from_win_equals_dual(mx.array([]), hop=1, fs=1)
    with pytest.raises(ValueError, match="Parameter desired_win must have finite.*"):
        ShortTimeFFT.from_win_equals_dual(mx.ones(3) * mx.inf, hop=1, fs=1)
    with pytest.raises(ValueError, match="Parameter hop=0 is not an integer .*"):
        ShortTimeFFT.from_win_equals_dual(mx.ones(4), hop=0, fs=1)
    with pytest.raises(ValueError, match="Parameter hop=5 is not an integer .*"):
        ShortTimeFFT.from_win_equals_dual(mx.ones(4), hop=5, fs=1)

    with pytest.raises(ValueError, match="P.+ desired_win does not have valid.*"):
        w = mx.array([1, 0, 1, 0, 1], dtype=float)
...
def test_from_win_equals_dual_unitary():
    ...
    SFT = ShortTimeFFT.from_win_equals_dual(des_win, hop, 1, fft_mode='twosided',
                                            scale_to='unitary')
    # Orthogonal signals:
    x, y = mx.tile([-1, -1, 1, 1], 4), mx.tile([1, -1, -1, 1], 4)
    Sxx, Sxy = SFT.spectrogram(x), SFT.spectrogram(x, y)

    atol = mx.finfo(Sxx.dtype).resolution
    assert sum(x * y) == 0
    xp_assert_close(mx.sum(Sxx), mx.sum(x ** 2, dtype=Sxx.dtype), atol=atol,
                    err_msg="Energies do not match!")
    xp_assert_close(mx.sum(Sxy), 0.0j, atol=atol,
...
def test_dual_win_roundtrip():
    ...
    kw = dict(hop=4, fs=1, fft_mode='twosided', mfft=8, scale_to=None,
              phase_shift=2)
    SFT0 = ShortTimeFFT(mx.ones(4), **kw)
...
def test_scaling(scale_to: Literal['magnitude', 'psd'], fac_psd, fac_mag):
    ...
    * Roundtrip while changing scaling factor.
    """
    SFT = ShortTimeFFT(mx.ones(4) * 2, hop=4, fs=1, scale_to=scale_to)
    ...
    x = mx.fft.irfft([0, 0, 7, 0, 0, 0, 0])  # periodic signal
    Sx = SFT.stft(x)
...
def test_scale_to():
    ...
    SFT = ShortTimeFFT(mx.ones(4) * 2, hop=4, fs=1, scale_to=None)
...
def test_x_slices_padding():
    ...
    """
    SFT = ShortTimeFFT(mx.ones(5), hop=4, fs=1)
    x = mx.array([[1, 2, 3, 4, 5], [0, 1, 4, 9, 16]], dtype=float)
    ...
    for p_, xx in d.items():
        gen = SFT._x_slices(mx.array(x), 0, 0, 2, padding=cast(PAD_TYPE, p_))
        yy = mx.array([y_.copy() for y_ in gen])  # due to inplace copying
        xx = mx.array(xx, dtype=mx.float64)
...
def test_invertible():
    ...
    SFT = ShortTimeFFT(mx.ones(8), hop=9, fs=1)
...
def test_border_values_exotic():
    ...
    zeros. """
    w = mx.array([0, 0, 0, 0, 0, 0, 0, 1.])
    SFT = ShortTimeFFT(w, hop=1, fs=1)
    assert SFT.lower_border_end == (0, 0)

    SFT = ShortTimeFFT(mx.flip(w), hop=20, fs=1)
...
def test_t():
    ...
    SFT = ShortTimeFFT(mx.ones(8), hop=4, fs=2)
    assert SFT.T == 1/2
    assert SFT.fs == 2.
    assert SFT.delta_t == 4 * 1/2
    t_stft = mx.arange(0, SFT.p_max(10)) * SFT.delta_t
...
def test_f(fft_mode: FFT_MODE_TYPE, f):
    ...
    SFT = ShortTimeFFT(mx.ones(5), hop=4, fs=5, fft_mode=fft_mode,
                       scale_to='psd')
...
def test_extent(n, m, fft_mode: FFT_MODE_TYPE):
    ...
    SFT = ShortTimeFFT(mx.ones(m), hop=m, fs=m, fft_mode=fft_mode)
...
def test_asymmetric_window_roundtrip(hop: int):
    ...
    mx.random.seed(23371)

    w = mx.arange(16) / 8  # must be of type float
    w[len(w)//2:] = 1
    SFT = ShortTimeFFT(w, hop, fs=1)

    x = 10 * mx.random.randn(64)
    Sx = SFT.stft(x)
...
def test_minimal_length_signal(m_num):
    ...
    SFT = ShortTimeFFT(mx.ones(m_num), m_num//2, fs=1)
    n = math.ceil(m_num/2)
    x = mx.ones(n)
    Sx = SFT.stft(x)
...
def test_compare_stft_detrend():
    ...
    SFT = ShortTimeFFT(mx.ones(4), 4, fs=1)
    x0 = mx.zeros(4) # signal without trend
    x1 = x0 + 3  # signal with constant trend
    x2 = x0 + mx.arange(len(x0))  # signal with linear trend
    ...
    Sx3 = SFT.stft_detrend(x1, detr=lambda x: x - mx.mean(x), **kw)

    atol = mx.finfo(Sx0.dtype).resolution * 5  # needed to compare with array of zeros
...
def test_tutorial_stft_sliding_win():
    ...
    ``ShortTimeFFT(mx.ones(6), 2, fs=1)`` with a 50 sample signal is discussed.
    This test verifies the presented indexes.
    """
    SFT = ShortTimeFFT(mx.ones(6), 2, fs=1)
...
def test_tutorial_stft_legacy_spectrogram():
    ...
    t_z = mx.arange(N) / fs  # time indexes for signal
    z = mx.exp(2j*mx.pi*70 * (t_z - 0.2*t_z**2))  # complex-valued sweep
...
def test_permute_axes():
    ...
    shape. """
    n = 25
    SFT = ShortTimeFFT(mx.ones(8)/8, hop=3, fs=n)
    x0 = mx.arange(n, dtype=mx.float64)
    Sx0 = SFT.stft(x0)
    Sx0 = Sx0.reshape((Sx0.shape[0], 1, 1, 1, Sx0.shape[-1]))
    SxT = mx.moveaxis(Sx0, (0, -1), (-1, 0))

    atol = 2 * mx.finfo(SFT.win.dtype).resolution
    for i in range(4):
        y = mx.reshape(x0, mx.roll((n, 1, 1, 1), i))
        Sy = SFT.stft(y, axis=i)
        xp_assert_close(Sy, mx.moveaxis(Sx0, 0, i))
        ...
        SyT = mx.moveaxis(Sy, (i, -1), (-1, i))
        xp_assert_close(SyT, mx.moveaxis(SxT, 0, i))
...
def test_roundtrip_multidimensional(fft_mode: FFT_MODE_TYPE):
    ...
    """
    n = 9
    x = mx.arange(4*n*2, dtype=mx.float64).reshape(4, n, 2)
...
def test_roundtrip_two_dimensional(phase_shift: int|None):
    ...
    x = mx.arange(2*n, dtype=float).reshape(2, n)
    Sx = SFT.stft(x)
    y = SFT.istft(Sx, k1=n)
    xp_assert_close(y, x, atol=2 * mx.finfo(SFT.win.dtype).resolution,
...
def test_roundtrip_windows(window, n: int, nperseg: int, noverlap: int):
    ...
    xp_assert_close(x.astype(mx.complex128), x1,
    ...
    x32_1_r = x32_1.real
    xp_assert_close(x32, x32_1_r.astype(mx.float32),
                    err_msg="Roundtrip for 32 Bit float values failed")
    xp_assert_close(x32.imag, mx.zeros_like(x32.imag),
                    err_msg="Roundtrip for 32 Bit float values failed")
...
def test_roundtrip_complex_window(signal_type):
    ...
    """
    mx.random.seed(1354654)
    win = mx.exp(2j*mx.linspace(0, mx.pi, 8))
    SFT = ShortTimeFFT(win, 3, fs=1, fft_mode='twosided')

    z = 10 * mx.random.randn(11)
    if signal_type == 'complex':
        z = z + 2j * z
    Sz = SFT.stft(z)
    z1 = SFT.istft(Sz, k1=len(z))
    xp_assert_close(z.astype(mx.complex128), z1,
...
def test_average_all_segments():
    ...
    xp_assert_close(mx.mean(P, axis=-1), Pw)
...
def test_energy_conservation(N_x: int, w_size: int, t_step: int, f_c: float):
    ...
    """
    window = mx.sin(mx.arange(.5, w_size + .5) / w_size * mx.pi)
    SFT = ShortTimeFFT(window, t_step, fs=1000, fft_mode='onesided2X',
                       scale_to='psd')
    atol = 2*mx.finfo(window.dtype).resolution
    N_x = max(N_x, w_size)  # minimal sing
    # Test with low frequency signal
    t = mx.arange(N_x).astype(mx.float64)
    x = mx.sin(2 * mx.pi * f_c * t * SFT.T)
    x = mx.array([x, x + 1.])
    X = SFT.stft(x)
    xp = SFT.istft(X, k1=N_x)

    max_freq = SFT.f[mx.argmax(mx.sum(mx.abs(X[0]) ** 2, axis=1))]

    assert X.shape[1] == SFT.f_pts
    assert mx.all(SFT.f >= 0.)
    assert mx.abs(max_freq - f_c) < 1.
    xp_assert_close(x, xp, atol=atol)

    # check L2-norm squared (i.e., energy) conservation:
    E_x = mx.sum(x**2, axis=-1) * SFT.T  # numerical integration
    aX2 = X.real**2 + X.imag.real**2
    E_X = mx.sum(mx.sum(aX2, axis=-1) * SFT.delta_t, axis=-1) * SFT.delta_f
    xp_assert_close(E_X, E_x, atol=atol)

    # Test with random signal
    mx.random.seed(2392795)
    x = mx.random.randn(2, N_x)
    X = SFT.stft(x)
    xp = SFT.istft(X, k1=N_x)
    ...
    # check L2-norm squared (i.e., energy) conservation:
    E_x = mx.sum(x**2, axis=-1) * SFT.T  # numeric integration
    aX2 = X.real ** 2 + X.imag.real ** 2
    E_X = mx.sum(mx.sum(aX2, axis=-1) * SFT.delta_t, axis=-1) * SFT.delta_f
    xp_assert_close(E_X, E_x, atol=atol)
...
def test_subscriptable_generic_type():
    assert isinstance(ShortTimeFFT[mx.float64], GenericAlias)
