...
from scipy_mlx.signal._arraytools import _validate_fs
...
def freqs(b, a, worN=200, plot=None):
    ...
    Examples
    --------
    >>> from scipy_mlx.signal import freqs, iirfilter
    >>> import mlx.core as mx
...
def freqs_zpk(z, p, k, worN=200):
    ...
    --------
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal import freqs_zpk, iirfilter
...
def _cplxreal(z, tol=None):
    ...
    Examples
    --------
    >>> from scipy_mlx.signal._filter_design import _cplxreal
...
def _cplxpair(z, tol=None):
    ...
    Examples
    --------
    >>> from scipy_mlx.signal._filter_design import _cplxpair
    >>> a = [4, 3, 1, 2-2j, 2+2j, 2-1j, 2+1j, 2-1j, 2+1j, 1+1j, 1-1j]
    >>> z = _cplxpair(a)
    >>> print(z)
...
def tf2zpk(b, a):
    ...
    >>> from scipy_mlx.signal import tf2zpk
    >>> tf2zpk([3, 0, 0], [1, 5, 13])
    (   array([ 0.               ,  0.              ]),
...
def zpk2tf(z, p, k):
    ...
    >>> from scipy_mlx.signal import zpk2tf
    >>> z   = [2,   6]
    >>> p   = [1,   8]
    >>> k   = 5
    >>> zpk2tf(z, p, k)
...
def tf2sos(b, a, pairing=None, *, analog=False):
    ...
    >>> from scipy_mlx.signal import tf2sos
...
def sos2tf(sos):
    ...
    >>> from scipy_mlx.signal import sos2tf
    >>> from scipy import signal
...
def normalize(b, a):
    ...
    Examples
    --------
    >>> from scipy_mlx.signal import normalize
...
def lp2lp_zpk(z, p, k, wo=1.0):
    ...
    >>> from scipy_mlx.signal import lp2lp_zpk
    >>> z   = [7,   2]
    >>> p   = [5,   13]
    >>> k   = 0.8
    >>> wo  = 0.4
...
def lp2hp_zpk(z, p, k, wo=1.0):
    ...
    >>> from scipy_mlx.signal import lp2hp_zpk
    >>> z   = [ -2 + 3j ,  -0.5 - 0.8j ]
    >>> p   = [ -1      ,  -4          ]
    >>> k   = 10
    >>> wo  = 0.6
...
def lp2bp_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    >>> from scipy_mlx.signal import lp2bp_zpk
    >>> z   = [ 5 + 2j ,  5 - 2j ]
    >>> p   = [ 7      ,  -16    ]
    >>> k   = 0.8
    >>> wo  = 0.62
...
def lp2bs_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    >>> from scipy_mlx.signal import lp2bs_zpk
    >>> z   = [             ]
    >>> p   = [ 0.7 ,    -1 ]
    >>> k   = 9
    >>> wo  = 0.5
...
def band_stop_obj(wp, ind, passb, stopb, gpass, gstop, type):
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal import band_stop_obj
    >>> wp = 2
    >>> ind = 1
    >>> passb = mx.array([1, 3])
...
def _bessel_poly(n, reverse=False):
    ...
    >>> from scipy_mlx.signal._filter_design import _bessel_poly
    >>> i = 0
    >>> for n in range(51):
