...
def czt_points(m, w=None, a=1+0j):
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal import czt_points
...
class CZT:
    ...
    >>> from scipy_mlx.signal import CZT
    >>> import mlx.core as mx
    >>> a = mx.random.rand(7)
    >>> b = mx.random.rand(7)
    >>> c = mx.random.rand(7)
...
class ZoomFFT(CZT):
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal import ZoomFFT
...
def czt(x, m=None, w=None, a=1+0j, *, axis=-1):
    ...
    >>> from scipy_mlx.fft import rfft, rfftfreq
    >>> from scipy_mlx.signal import czt, czt_points
...
def zoom_fft(x, fn, m=None, *, fs=2, endpoint=False, axis=-1):
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal import zoom_fft
    >>> t = mx.linspace(0, 1, 1021)
    >>> x = mx.cos(2*mx.pi*15*t) + mx.sin(2*mx.pi*17*t)
    >>> f1, f2 = 5, 27
