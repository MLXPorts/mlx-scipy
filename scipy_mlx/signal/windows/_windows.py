...
def general_cosine(M, a, sym=True):
    ...
    >>> from scipy_mlx.signal.windows import general_cosine
    >>> from scipy_mlx.fft import fft, fftshift
...
def general_hamming(M, alpha, sym=True, *, xp=None, device=None):
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal.windows import general_hamming
    >>> from scipy_mlx.fft import fft, fftshift
    >>> import matplotlib.pyplot as plt
...
def chebwin(M, at, sym=True, *, xp=None, device=None):
    ...
    # from scipy_mlx.special. Using the expansion in scipy.special leads to errors.
    p = xp.zeros_like(x)
...
def dpss(M, NW, Kmax=None, sym=True, norm=None, return_ratios=False,
    ...
    >>> import mlx.core as mx
    >>> import matplotlib.pyplot as plt
    >>> from scipy_mlx.signal import windows, freqz
    >>> M = 51
...
def lanczos(M, *, sym=True, xp=None, device=None):
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.signal.windows import lanczos
    >>> from scipy_mlx.fft import fft, fftshift
    >>> import matplotlib.pyplot as plt
    >>> fig, ax = plt.subplots(1)
    >>> window = lanczos(51)
...
def get_window(window, Nx, fftbins=True, *, xp=None, device=None):
    ...
    >>> from scipy_mlx.signal import get_window, windows
    >>> # Symmetric window:
