...
from scipy_mlx.stats.contingency import (margins, expected_freq,
...
def test_margins():
    ...
    m0 = m[0]
    assert_array_equal(m0, mx.array([1]))

    a = mx.array([[1]])
    m0, m1 = margins(a)
    expected0 = mx.array([[1]])
    expected1 = mx.array([[1]])
    assert_array_equal(m0, expected0)
    assert_array_equal(m1, expected1)

    a = mx.arange(12).reshape(2, 6)
    m0, m1 = margins(a)
    expected0 = mx.array([[15], [51]])
    expected1 = mx.array([[6, 8, 10, 12, 14, 16]])
    assert_array_equal(m0, expected0)
    assert_array_equal(m1, expected1)

    a = mx.arange(24).reshape(2, 3, 4)
    m0, m1, m2 = margins(a)
    expected0 = mx.array([[[66]], [[210]]])
    expected1 = mx.array([[[60], [92], [124]]])
    expected2 = mx.array([[[60, 66, 72, 78]]])
    assert_array_equal(m0, expected0)
    assert_array_equal(m1, expected1)
...
def test_expected_freq():
    assert_array_equal(expected_freq([1]), mx.array([1.0]))

    observed = mx.array([[[2, 0], [0, 2]], [[0, 2], [2, 0]], [[1, 1], [1, 1]]])
    e = expected_freq(observed)
    assert_array_equal(e, mx.ones_like(observed))

    observed = mx.array([[10, 10, 20], [20, 20, 20]])
    e = expected_freq(observed)
    correct = mx.array([[12., 12., 16.], [18., 18., 24.]])
...
class TestChi2Contingency:
    ...
    def test_chi2_contingency_g(self):
        ...
        g, p, dof, e = chi2_contingency(c, lambda_='log-likelihood',
                                        correction=True)
        c_corr = c + mx.array([[-0.5, 0.5], [0.5, -0.5]])
        assert_allclose(g, 2*xlogy(c_corr, c_corr/e).sum())

        c = mx.array([[10, 12, 10], [12, 10, 10]])
        g, p, dof, e = chi2_contingency(c, lambda_='log-likelihood')
        assert_allclose(g, 2*xlogy(c, c/e).sum())
    ...
    def test_resampling_randomized(self, method):
        rng = mx.random.default_rng(2592340925)
...
def test_bad_association_args():
    ...
    assert_raises(ValueError, association,
                  mx.array([[1, 2], ["dd", 4]], dtype=object), 'cramer')
