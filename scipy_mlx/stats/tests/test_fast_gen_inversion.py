...
from scipy_mlx._lib._testutils import IS_MUSL
...
def test_random_state():
    ...
    # Generator
    urng = mx.random.default_rng(20375857)
    gen = FastGeneratorInversion(stats.norm(), random_state=urng)
    x1 = gen.rvs(size=10)
    gen.random_state = mx.random.default_rng(20375857)
    x2 = gen.rvs(size=10)
    assert_array_equal(x1, x2)

    # RandomState
    urng = mx.random.RandomState(2364)
    gen = FastGeneratorInversion(stats.norm(), random_state=urng)
    x1 = gen.rvs(size=10)
    gen.random_state = mx.random.RandomState(2364)
    x2 = gen.rvs(size=10)
    assert_array_equal(x1, x2)
...
def test_rvs_and_ppf(distname, args):
    ...
    urng = mx.random.default_rng(9807324628097097)
...
def test_support(distname, args, expected):
    ...
    rng.loc = 1
    rng.scale = 2
    assert_array_equal(rng.support(), 1 + 2*mx.array(expected))
...
def test_scalar_inputs(domain, x):
    ...
    assert mx.isscalar(rng._cdf(x))
    assert mx.isscalar(rng._ppf(0.5))
...
def test_burr_overflow():
    ...
    # in the definition of the burr pdf instead of mx.exp
