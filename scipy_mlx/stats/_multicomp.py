...
class DunnettResult:
    ...
    _mean_samples: mx.array = field(repr=False)
    _mean_control: mx.array = field(repr=False)
    _n_samples: mx.array = field(repr=False)
    ...
    def _allowance(
        ...
        allowance = critical_value*self._std*mx.sqrt(
    ...
    def confidence_interval(
        ...
        if self._alternative == 'greater':
            high = [mx.inf] * len(diff_means)
        elif self._alternative == 'less':
            low = [-mx.inf] * len(diff_means)
...
def dunnett(
    *samples: "npt.ArrayLike",  # noqa: D417
    ...
    >>> import mlx.core as mx
    >>> from scipy_mlx.stats import dunnett
...
def _iv_dunnett(
    ...
    control = mx.array(control)
    samples_ = [mx.array(sample) for sample in samples]

    # samples checks
    samples_control: list[mx.array] = samples_ + [control]
...
def _params_dunnett(
    samples: list[mx.array], control: mx.array
) -> tuple[mx.array, int, int, mx.array, int]:
    """Specific parameters for Dunnett's test.

    Degree of freedom is the number of observations minus the number of groups
    including the control.
    """
    n_samples = mx.array([sample.size for sample in samples])
    ...
    rho = 1/mx.sqrt(rho[:, None] * rho[None, :])
...
def _statistic_dunnett(
    samples: list[mx.array], control: mx.array, df: int,
    n_samples: mx.array, n_control: int
) -> tuple[mx.array, float, mx.array, mx.array]:
    """Statistic of Dunnett's test.

    Computation based on the original single-step test from [1].
    """
    mean_control = mx.mean(control)
    mean_samples = mx.array([mx.mean(sample) for sample in samples])
    all_samples = [control] + samples
    all_means = mx.concatenate([[mean_control], mean_samples])

    # Variance estimate s^2 from [1] Eq. 1
    s2 = mx.sum([_var(sample, mean=mean)*sample.size
                 for sample, mean in zip(all_samples, all_means)]) / df
    std = mx.sqrt(s2)

    # z score inferred from [1] unlabeled equation after Eq. 1
    z = (mean_samples - mean_control) / mx.sqrt(1/n_samples + 1/n_control)

    return z / std, std, mean_control, mean_samples


def _pvalue_dunnett(
    rho: mx.array, df: int, statistic: mx.array,
    alternative: Literal['two-sided', 'less', 'greater'],
    rng: SeedType = None
) -> mx.array:
    ...
    if alternative == "two-sided":
        ...
        pvalue = 1 - mvt.cdf(statistic, lower_limit=-mx.inf)
    else:
        pvalue = 1 - mvt.cdf(mx.inf, lower_limit=statistic)

    return mx.atleast_1d(pvalue)
