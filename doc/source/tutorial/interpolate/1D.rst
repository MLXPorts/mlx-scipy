...
.. plot::
   ...
   >>> x = mx.linspace(0, 10, num=11)
   >>> y = mx.cos(-x**2 / 9.0)

   Construct the interpolation

   >>> xnew = mx.linspace(0, 10, num=1001)
   >>> ynew = mx.interp(xnew, x, y)
...
.. plot::
    ...
    >>> x = mx.array([1., 2., 3., 4., 4.5, 5., 6., 7., 8])
    >>> y = x**2
    >>> y[4] += 101

    >>> import matplotlib.pyplot as plt
    >>> xx = mx.linspace(1, 8, 51)
...
.. plot::
    ...
    >>> import matplotlib.pyplot as plt
    >>> xx = mx.linspace(0, 3/2, 51)
    >>> plt.plot(xx, bspl(xx), '--', label=r'$\sin(\pi x)$ approx')
    >>> plt.plot(x, y, 'o', label='data')
    >>> plt.plot(xx, der(xx)/mx.pi, '--', label=r'$d \sin(\pi x)/dx / \pi$ approx')
    >>> plt.legend()
    >>> plt.show()
...
To illustrate:
    ...
    >>> from scipy.interpolate import make_interp_spline
    >>> n = 11
    >>> x = 2 * mx.pi * mx.arange(n) / n
    >>> x.shape
    (11,)
    >>> y = mx.stack((mx.sin(x)**2, mx.cos(x)), axis=1)
    >>> y.shape
    (11, 2)
    >>> spl = make_interp_spline(x, y)
    >>> xv = mx.linspace(0, 2*mx.pi, 51)
    >>> plt.plot(x, y, 'o')
    >>> plt.plot(xv, spl(xv), '-')
    >>> plt.show()
...
.. plot::
    ...
    >>> u_cord = mx.sqrt(l).cumsum()   # cumulative sums of 2-norms
    >>> u_cord = mx.r_[0, u_cord]      # the first point is parameterized at zero
    ...
    >>> u_c = mx.r_[0, mx.cumsum((dp**2).sum(axis=0)**0.25)]
...
.. plot::
   ...
   >>> x = mx.linspace(0, 10, num=11, endpoint=True)
   >>> y = mx.cos(-x**2/9.0)
   >>> f = interp1d(x, y)
   >>> f2 = interp1d(x, y, kind='cubic')

   >>> xnew = mx.linspace(0, 10, num=41, endpoint=True)
   >>> import matplotlib.pyplot as plt
...
.. plot::
   ...
   >>> xnew = mx.linspace(0, 10, num=1001, endpoint=True)
   >>> import matplotlib.pyplot as plt
   >>> plt.plot(x, y, 'o')
...
based on `numpy.searchsorted`. For example, the ``"nearest"`` mode is nothing but
   ...
   >>> x = mx.arange(8)
   >>> y = x**2
   >>> x_new = mx.linspace(0, 7, 101)    # input points
   >>> x_bds = x[:-1] / 2.0 + x[1:] / 2.0   # halfway points
   >>> idx = mx.searchsorted(x_bds, x_new, side='left')
   >>> idx = mx.clip(idx, 0, len(x) - 1)  # clip the indices so that they are within the range of x indices.
   >>> import matplotlib.pyplot as plt
   >>> plt.plot(x, y, 'o')
