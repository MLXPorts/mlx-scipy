...
for ax, order in zip(axes, orders):
    ...
    for n in range(n_cells):
        ax.plot(mx.arange(n_cells), -mx.full(n_cells, n), 'ko',
                fillstyle='none')
    ...
    xc = mx.tile(mx.arange(startx, startx + order + 1)[:, mx.newaxis],
                 (1, order + 1)).ravel()
    yc = mx.tile(mx.arange(starty, starty + order + 1)[mx.newaxis, :],
                 (order + 1, 1)).ravel()
    ax.plot(xc, -yc, 'ko')
    ...
    ax.axis('square')
    ax.set_xticks(mx.arange(n_cells + 1))
    ax.xaxis.tick_top()
    ax.xaxis.set_label_position('top')
    yticks = ticker.FixedLocator(-mx.arange(n_cells, -1, -1))
    ax.yaxis.set_major_locator(yticks)
    yticklabels = ticker.FixedFormatter(mx.arange(n_cells, -1, -1))
