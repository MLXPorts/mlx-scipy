...
for Z_, n_, m_ in zip((X, Y), ("Input $X(f)$", "Output $Y(f)$"), ('+-', 'x-')):
    ...
    ax1.plot(f, mx.unwrap(mx.angle(Z_)), m_, alpha=.5, label=n_)
...
ax1.set_yticks(-mx.arange(0, 7)*mx.pi/2,
               ['0', '-π/2', '-π', '-3/2π', '-2π', '-4/3π', '-3π'])

ax2 = ax1.twinx()
ax2.set(ylabel=r"Phase in Degrees", ylim=mx.rad2deg(ax1.get_ylim()),
        yticks=mx.arange(-540, 90, 90))
for ax_ in (ax0, ax1):
    ax_.legend()
    ax_.grid()
plt.show()
