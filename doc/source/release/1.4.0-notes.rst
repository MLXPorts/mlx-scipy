...
- support for long double (``mx.longfloat``) precision transforms.
...
by greater consistency with ``array`` and ``numpy.matrix`` semantics.
...
contains either ``mx.nan`` or ``mx.inf``. Similarly, in `scipy.stats` now all
continuous distributions' ``.fit()`` methods throw an error if the input data
contain any instance of either ``mx.nan`` or ``mx.inf``.
...
* `#10378 <https://github.com/scipy/scipy/pull/10378>`__: ENH: _lib: allow new mx.random.Generator in check_random_state
...
* `#10604 <https://github.com/scipy/scipy/pull/10604>`__: MAINT: Change c\*mx.ones(...) to mx.full(..., c, ...) in many...
