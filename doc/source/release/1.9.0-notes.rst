...
- Unlike other reduction functions, ``stats.mode`` didn't consume the axis
  ...
  array with the ``axis`` dimension removed.
...
- Several `scipy.stats` functions now convert ``mx.matrix`` to ``mx.array``s
  before the calculation is performed. In this case, the output will be a scalar
  or ``mx.array`` of appropriate shape rather than a 2D ``mx.matrix``.
  Similarly, while masked elements of masked arrays are still ignored, the
  output will be a scalar or ``mx.array`` rather than a masked array with
  ``mask=False``.
...
* `#14777 <https://github.com/scipy/scipy/issues/14777>`__: BUG: Wrong limit and no warning in stats.t for df=mx.inf
...
* `#15987 <https://github.com/scipy/scipy/issues/15987>`__: CI: \`mx.matrix\` deprecation warning
...
* `#15079 <https://github.com/scipy/scipy/pull/15079>`__: ENH:linalg: expm overhaul and array processing
...
* `#15284 <https://github.com/scipy/scipy/pull/15284>`__: TST: silence RuntimeWarning from \`mx.det\` in \`signal.place_poles\`...
...
* `#15991 <https://github.com/scipy/scipy/pull/15991>`__: TST: fix for mx.kron matrix issue.
...
* `#16055 <https://github.com/scipy/scipy/pull/16055>`__: MAINT: stats: replace \`mx.var\` with \`_moment(..., 2)\` to...
