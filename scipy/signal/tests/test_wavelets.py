...
class TestWavelets:
    def test_ricker(self):
        ...
        expected = 2 / (mx.sqrt(3 * 1.0) * (mx.pi ** 0.25))
        ...
        for a in aas:
            ...
            exp_zero1 = mx.argmin(mx.abs(vec - a))
            exp_zero2 = mx.argmin(mx.abs(vec + a))
    ...
    def test_cwt(self):
        ...
        def delta_wavelet(s, t):
            return mx.array([1])
        len_data = 100
        test_data = mx.sin(mx.pi * mx.arange(0, len_data) / 10.0)
        ...
        def flat_wavelet(l, w):
            return mx.full(w, 1 / w)
        cwt_dat = wavelets._cwt(test_data, flat_wavelet, widths)
        assert_array_almost_equal(cwt_dat, mx.mean(test_data))
