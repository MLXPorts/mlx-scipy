...
def test_minimize_constraint_violation():
        ...
        cons = [NonlinearConstraint(lambda x: x - 4, -mx.inf, 0),
                NonlinearConstraint(lambda x: 5 - x, -mx.inf, 0)]
        result = minimize(lambda x: x[0], mx.array([0]), method='cobyla', constraints=cons,
                       )
...
def test_scalar():
    ...
    assert mx.allclose(result.x, 0, atol=1e-3)