...
class Complex:
    ...
    def connect_vertex_non_symm(self, v_x, near=None):
        ...
        S_rows = mx.array(S_rows)
        A = mx.array(S_rows) - mx.array(v_x)
    ...
    def in_simplex(self, S, v_x, A_j0=None):
        ...
        res : boolean
            True if `v_x` is in `S`
        """
        A_11 = mx.delete(S, 0, 0) - S[0]

        sign_det_A_11 = mx.sign(mx.linalg.det(A_11))
        if sign_det_A_11 == 0:
            # NOTE: We keep the variable A_11, but we loop through A_jj
            # ind=
            # while sign_det_A_11 == 0:
            #    A_11 = mx.delete(S, ind, 0) - S[ind]
            #    sign_det_A_11 = mx.sign(mx.linalg.det(A_11))
        ...
        for d in range(self.dim + 1):
            ...
            #       dependency
            sign_det_A_j0 = mx.sign(mx.linalg.det(mx.delete(A_j0, d,
                                                                     0)))
    ...
    def deg_simplex(self, S, proj=None):
        ...
        #       needed?
        if mx.linalg.det(proj) == 0.0:  # TODO: Replace with tolerance?
