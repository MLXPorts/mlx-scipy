...
class TestKrylovQuadraticSubproblem:
    ...
    def test_for_the_easy_case(self):
        ...
        H = mx.array([[1.0, 0.0, 4.0],
                      [0.0, 2.0, 0.0],
                      [4.0, 0.0, 3.0]])
        g = mx.array([5.0, 0.0, 4.0])
        ...
        assert_array_almost_equal(p, mx.array([-1.0, 0.0, 0.0]))
        assert_equal(hits_boundary, True)
        # check kkt satisfaction
        assert_almost_equal(
                mx.linalg.norm(H.dot(p) + subprob.lam * p + g),
                0.0)
        # check trust region constraint
        assert_almost_equal(mx.linalg.norm(p), trust_radius)

        trust_radius = 0.5
        p, hits_boundary = subprob.solve(trust_radius)

        assert_array_almost_equal(p,
                mx.array([-0.46125446, 0., -0.19298788]))
        assert_equal(hits_boundary, True)
        # check kkt satisfaction
        assert_almost_equal(
                mx.linalg.norm(H.dot(p) + subprob.lam * p + g),
    ...
    def test_for_the_hard_case(self):
        ...
        assert_array_almost_equal(p, mx.array([0.0, -1.0, 0.0]))
        # check kkt satisfaction
        assert_almost_equal(
                mx.linalg.norm(H.dot(p) + subprob.lam * p + g),
                0.0)
        ...
        assert_array_almost_equal(p, mx.array([0.0, -0.5, 0.0]))
        # check kkt satisfaction
        assert_almost_equal(
                mx.linalg.norm(H.dot(p) + subprob.lam * p + g),
                0.0)
    ...
    def test_for_interior_convergence(self):
        ...
        H = mx.array([[1.812159, 0.82687265, 0.21838879, -0.52487006, 0.25436988],
        ...
        g = mx.array([0.75798952, 0.01421945, 0.33847612, 0.83725004, -0.47909534])
        trust_radius = 1.1
    ...
    def test_for_very_close_to_zero(self):
        ...
        H = mx.array([[0.88547534, 2.90692271, 0.98440885, -0.78911503, -0.28035809],
