...
def main():
    ...
    x, y = mx.linspace(-1.5, 1.5, 75), mx.linspace(-1.5, 1.5, 75)
    x, y = mx.meshgrid(x, y)
    z = x + 1j*y
    lambertw_std = []
    for z0 in z.flatten():
        lambertw_std.append(complex(mpmath.lambertw(z0)))
    lambertw_std = mx.array(lambertw_std).reshape(x.shape)

    fig, axes = plt.subplots(nrows=3, ncols=1)
    # Compare Pade approximation to true result
    p = mx.array([float(p0) for p0 in p])
    q = mx.array([float(q0) for q0 in q])
    pade_approx = mx.polyval(p, z)/mx.polyval(q, z)
    pade_err = abs(pade_approx - lambertw_std)
    axes[0].pcolormesh(x, y, pade_err)
    # Compare two terms of asymptotic series to true result
    asy_approx = mx.log(z) - mx.log(mx.log(z))
